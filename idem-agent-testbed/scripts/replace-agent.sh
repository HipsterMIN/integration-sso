...
#
#   # 특정 JAR 지정
#   ./scripts/replace-agent.sh tomcat-9 /path/to/idem-agent-1.0.0-all.jar
#
# 동작:
#   1. 지정 JAR(또는 최신 빌드 JAR)를 ./agent/ 디렉토리에 복사
#   2. 심볼릭 링크 idem-agent-current.jar 갱신
#   3. 대상 컨테이너 재시작
##############################################################################
...
find_agent_jar() {
    ...
    # idem-agent 빌드 결과에서 최신 fat-JAR 탐색
    local build_jar
    build_jar=$(find "$PROJECT_ROOT/idem-agent/build/libs" \
        -name "idem-agent-*-all.jar" 2>/dev/null | sort -V | tail -1)
    ...
    # ./agent/ 디렉토리에서 탐색
    local agent_jar
    agent_jar=$(find "$AGENT_DIR" -name "idem-agent-*-all.jar" 2>/dev/null | sort -V | tail -1)
...
deploy_jar() {
    ...
    # 심볼릭 링크 갱신
    local symlink="$AGENT_DIR/idem-agent-current.jar"
    ln -sf "$jar_name" "$symlink"
    log_ok "심볼릭 링크 갱신: idem-agent-current.jar → $jar_name"
...
main() {
    ...
    if [[ -z "$source_jar" ]]; then
        ...
        log_warn "  cd $(realpath "$PROJECT_ROOT") && ./gradlew :idem-agent:agentJar"
        log_warn "  또는 JAR 경로를 직접 지정:"
        log_warn "  $0 all /path/to/idem-agent-xxx-all.jar"
        exit 1
    fi
