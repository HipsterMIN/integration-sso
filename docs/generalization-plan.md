...
- ✅ **계정·역할·범위**: `ido.admin_user`(V25) — PBKDF2 비밀번호, 역할 `SYSTEM_ADMIN`/`POLICY_ADMIN`/`AUDITOR`, `tenant_code`(있으면 그 테넌트의 기관만: 목록 필터·단건 403·새 프로파일의 `service.tenant` 검사 — v0.5 결정 (4) 최소 구현), 상태·잠금·비밀번호 이력(3)·봉인된 TOTP 비밀. 부트스트랩: 관리자 0명이면 `IDEM_ADMIN_BOOTSTRAP_PASSWORD` 로 첫 `SYSTEM_ADMIN`(첫 로그인에서 변경 강제), `prod`/`stage` 에서 비면 기동 거부
- ✅ **인증**: 비밀번호 + TOTP(RFC 6238, `CryptoProvider.hmacSha1` 하나로만 SHA-1 을 연다 — `CryptoBoundaryGuardTest` 가 잡아 준 경계) 2단계, 첫 로그인에서 등록(`secret`/`otpauthUri` 1회 노출), 비밀은 `IDEM_ADMIN_SECRET_KEY` 로 AES-GCM 봉인. Redis 세션 유휴 15분·절대 8시간·동시 1, 쿠키 `idemAdminSid` HttpOnly·Secure·SameSite=Strict, 모든 쓰기(로그인 포함)에 `X-Requested-With` 필수(CSRF). 실패 5회 → 15분 잠금(423), 비밀번호 정책(10자·3종·사용자명·이력). 로그인·2단계 경로는 레이트리밋 대상
