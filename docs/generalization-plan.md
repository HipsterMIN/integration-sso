...
**진행 기록 (2026-09-24, S6 PR-1: Keycloak 을 숨긴 OIDC_RP 정식화)** — 구현 PR. 설계 근거는 `wiki/adr/ADR-014-hidden-keycloak-oidc-front.md`.
- ✅ **프로토콜 모델**: `IntegrationType.OIDC_RP`(V23 CHECK), 프로파일 `protocol.oidc { redirectUris, postLogoutRedirectUris, backchannelLogoutUri, clientAuthMethod }`(스키마 `if/then` 으로 OIDC_RP 필수, 의미 검증: 와일드카드·fragment 금지, 다른 유형의 oidc 블록 거부). OIDC_RP 기관에는 Handoff 발급이 닫힌다(`E-IDO-121`)
- ✅ **client 프로비저닝(hub)**: 프로파일 저장과 같은 트랜잭션에서 Keycloak client `idem-svc-{code}` 를 만들거나 맞춘다 — confidential·code 만·PKCE S256·`fullScopeAllowed=false`·매퍼 `identity_provider`/`idem_service`. 실패는 `E-IDO-122` 로 저장을 되돌린다(통합 테스트로 확인). 유형이 바뀌면 비활성으로 남긴다. `GET /admin/services/{code}/oidc-client`, `POST …/oidc-client/secret`(한 번만 보인다). 자격은 서비스 계정 `idem-provisioner`(realm-management `manage-clients`·`view-clients`, `fullScopeAllowed=true` 여야 토큰에 역할이 실린다) — 관리자 비밀번호는 앱이 쓰지 않는다
- ✅ **gate OIDC 프런트**: issuer = `{IDEM_PUBLIC_URL_GATE}/realms/{realm}`(Keycloak `KC_HOSTNAME_URL` 과 동일), 정직한 Discovery(code·PKCE S256·RS256·secret basic/post·refresh 만), `/realms/**`·`/resources/**` 투명 프록시(302·쿠키 통과, 내부 `Location` 재작성, `X-Forwarded-*`), auth 사전검사(`idem-svc-*`·code·PKCE), **토큰 교환 시 hub 판정** → 거부면 `403 access_denied`(`error_description` = `E-IDO-1xx …`) + refresh_token 로그아웃, hub 불가면 `503 temporarily_unavailable`, 허용 판정은 (client, sub) Redis 캐시 → userinfo 에 `idem_service·idem_state·idem_subject·idem_subject_scheme·idem_roles·idem_assigned·idem_user_id`. 종전 `OidcDiscoveryController`(Keycloak URL·기능 목록 그대로 노출, 미사용 프록시) 는 제거
- ✅ **hub 내부 판정 API** `POST /api/internal/v1/oidc-rp/access`(X-Internal-Sig): client → 서비스, `sub` → registry 사용자(찾거나 등록, 장애는 거부 E-IDO-116), Handoff 와 **같은 `PolicyEngine`**(ASSIGNMENT 포함) 평가, `state/agencySubjectId/roles/assigned` 어휘 동일(`PolicyEngine.resolveAgencySubjectId` 공개)
- ✅ **실 Keycloak 끝-끝 (로컬, Keycloak 24.0.5 + registry·hub·gate 실기동 + Chromium)**: 프로파일 PUT → Keycloak 에 client 생성 확인 → secret 회전 → 브라우저 PKCE 로그인(gate 가 프록시한 Keycloak 로그인 화면, 브라우저는 8180 에 닿지 않음) → code → gate 토큰 교환(hub 판정 통과) → `id_token.iss=http://localhost:8081/realms/onepass`, `azp=idem-svc-AG_E2E`, `idem_service` → userinfo `idem_state=APPROVED`·`idem_subject`(PAIRWISE_HMAC)·`idem_user_id`. 거부 경로: `allowedProviders:[NICE]` 로 바꾸면 토큰 교환이 `403 access_denied "E-IDO-104 허용되지 않은 제공자: UNKNOWN"` 이고 Keycloak 세션 수가 늘지 않는다(revoke 동작), 허용 경로는 +1
- ✅ **끝-끝에서 잡은 기존 결함 4건**: (1) `realm-export.json` 의 secret 자리표시자 `${env.X:change-me}` 를 Keycloak 24 가 치환하지 않아 **모든 설치본의 `q-sign-client`·`ido-client` secret 이 문자 그대로 `change-me`** 였다 → `${X}` 로 수정(기존 설치본은 `keycloak-data` 재import 또는 콘솔에서 1회 정정, `docs/install.md` §7) (2) gate·registry 가 `IDEM_KAFKA_ENABLED=false` 로 기동하지 못했다(모듈 한정자 `qsignKafkaTemplate`/`qimKafkaTemplate` 로 주입해 `DisabledKafkaTemplate` 을 못 받음 — D1 단일 설치본은 hub 만 CI 가 기동 확인) → 한정자 제거 (3) gate 가 `RedisTemplate<String,Object>` 빈 없이 `PkceService` 를 두어 기동 실패 → `GateRedisConfig` (4) gate `SecurityHeadersFilter` 의 CSP(`form-action 'none'`)가 프록시된 로그인 화면에 덧씌워져 브라우저가 폼 제출을 거부 → `/realms/**`·`/resources/**` 는 필터 제외. 추가로 gate `KeycloakAuthUrlController` 가 `X-Internal-Sig` 를 받기만 하고 검증하지 않던 것을 검증으로 고침
...
**진행 기록 (2026-09-25, S6 PR-2)** — 구현 PR.
- ✅ **SLO 수리 (①)**: FE 세션이 Keycloak `sub`·`sid` 를 기억한다(`FeSession.idpSub/idpSid`, Redis 역인덱스 `fe:idp-sid:*`·`fe:idp-sub:*`; gate 콜백과 hub Keycloak 브로커 경로가 채운다). hub SLO 는 이제 `qimUserId` 대신 그 값을 gate 에 넘기고, gate `KeycloakLogoutService` 는 서비스 계정 `idem-session-manager`(realm-management `view-users`·`manage-users`)로 `DELETE /admin/realms/{realm}/sessions/{sid}`(정확히 그 세션) 또는 `POST /users/{sub}/logout` 을 부른다. 종전 구현은 서비스 계정 없는 `q-sign-client`·username 조회·비표준 DELETE 로 한 번도 성공할 수 없었다
- ✅ **Back-Channel Logout 수신 (①)**: gate `POST /api/v1/oidc/backchannel-logout` — `logout_token` 검증(RS256·iss·aud=내부 client·events·nonce 없음·sub/sid) → hub `POST /api/internal/v1/session/idp-logout`(FE 세션 만료) + 판정 캐시 삭제. realm-export 의 `q-sign-client`·`ido-client` 가 이 URL 을 가리키므로 관리자 종료·RP-Initiated Logout·SLO 어느 쪽이든 FE 세션이 정리된다. RP-Initiated Logout(`end_session_endpoint`)은 gate 가 `id_token_hint` 를 검증해 같은 정리를 먼저 하고 Keycloak 에 넘긴다
- ✅ **tenant-sample 표준 RP 경로 (②)**: `agency-stub.protocol=OIDC_RP` 스위치 + `/agency/oidc/{login,callback,logout,backchannel-logout}`. 라이브러리 없이 Discovery·PKCE S256·state·nonce·id_token(RS256/JWKS·iss·aud·exp·nonce)·userinfo·logout_token 검증을 다 보여 주는 참조 구현. userinfo 의 `idem_*` 를 Handoff 어설션 모양으로 바꿔 **같은 세션 저장소·쿠키** 를 쓴다 — 기관 코드에서 바뀌는 것은 "어떻게 들어오느냐" 뿐. 완료 기준 충족: `protocol` 만 바꿔 Handoff·OIDC_RP 두 방식 통과(단위 4 + 실 끝-끝)
- ✅ **실 Keycloak 끝-끝 (registry·hub·gate·tenant-sample 실기동 + Chromium)**: (a) RP 로그인 → APPROVED·AGSID → RP-Initiated Logout → gate 가 캐시 삭제·hub 통지 → Keycloak 세션 4→3 → Keycloak 이 RP 에 Back-Channel Logout (b) 로그인 뒤 hub/SLO 가 하는 그대로 gate 내부 API 에 `sid` 로 종료 요청 → `REVOKED_SESSION` → Keycloak 세션 -1 → Keycloak 이 RP 에 Back-Channel Logout → RP 세션 `invalidated=1`. 즉 로그아웃이 Keycloak·FeSession·RP 세 곳에서 끝난다
- ✅ **끝-끝에서 잡은 기존 결함**: tenant-sample `AgencySessionService` 가 `java.time.Instant` 를 JDBC 에 그대로 넘겨 PostgreSQL 에서 세션 INSERT 가 항상 실패했다(Handoff 진입도 같은 경로 — PostgreSQL E2E 테스트는 `@Tag("e2e")` 라 CI 에서 돈 적이 없음) → `Timestamp.from` 으로 수정
- ✅ **SAML_SP 설계 (⑤)**: `docs/saml-sp-design.md` — Keycloak SAML client 프로비저닝 + 정책 강제는 authenticator SPI 로(브라우저 POST 라 백채널 판정 지점이 없음)
- ✅ 검증: hub 495 · gate 106 · tenant-sample 84+4 · 나머지 모듈 무변경
- ⏭ **PR-3 로 이월**: ③ `IntegrationProtocol` SPI 로 Handoff 전략 승격 ④ `idp-hint-mapping` 프로파일화(둘 다 기능 변화 없는 구조 정리라 D3 뒤에) · Keycloak `sid` 미저장 세션(본인확인 SPI 경로)의 SLO 는 FE 세션만 끝난다(IdP 세션 없음 — 정상) · hub FE 로그인(브로커 경로)의 백채널 끝-끝은 단위 테스트로만 확인(`FeSessionServiceImplIdpTest`·`SloServiceImplIdpTest`)
...
**진행 기록 (2026-09-25, D3)** — 구현 PR.
- ✅ **수리 9건 (D3-a)**: ① gate 잠금 — `LockRepositoryImpl` 이 임계치(5회)에서 실제로 잠그고(`unlockAt` = now+`qsign.lock.duration-minutes`), `AuthServiceImpl`·`KeycloakCallbackService` 가 실패 시 `incrementAttempt`, 성공 시 `unlock`, 잠금 키 `(identifierHash, providerCode)`(종전 `(providerCode, providerCode)` 는 아무도 잠기지 않는 키) ② CAST 단일 사용 — `GETSET` 원자 소비(재사용·만료 경합 제거) ③ JWKS — `@Cacheable`(캐시 매니저 없이 매번 Keycloak 호출) 대신 TTL 캐시(gate `qsign.keycloak.jwks-cache-ttl-seconds`, hub `ido.keycloak.jwks-cache-ttl-seconds`) ④ 세션 정책 — `policy.session` 이 Handoff 페이로드 `sessionPolicy`·OIDC userinfo `idem_session_policy` 로 기관에 전달되고, Handoff 발급 시 `FeSessionPolicyEnforcer` 가 Idem FE 세션의 유휴·절대 만료를 줄이고 동시 세션 초과분을 오래된 순으로 끝낸다(늘리지는 않는다) ⑤ 상태 변경 전파 — `QimEventConsumer.handle` 이 정지·탈퇴 이벤트에 살아 있는 FE 세션을 전부 끝내고, Kafka 없는 설치는 registry 의 읽기 전용 피드 `GET /api/v1/internal/events`((created_at, event_id) 키셋)를 hub `RegistryOutboxPoller` 가 폴링한다(`ido.qim-events.poll.enabled`, Kafka 꺼짐 파생 기본값 true, 워터마크 Redis, 독약 이벤트는 max-failures 뒤 FAILED 기록) ⑥ registry PII 삭제 두 경로 모두 `subject_key` 포함 ⑦ `updateStatus` 상태 검증·탈퇴 후 전이 차단·이벤트 타입이 새 상태를 따름(종전 항상 USER_SUSPENDED) ⑧ gate 자체 Keycloak 로그인 PKCE(S256, verifier 는 state 엔트리에만) + id_token `iss` 검증(내부/공개 issuer) ⑨ authz 미사용 RLS 정책·활성화 제거(V4)
- ✅ **회귀 테스트가 드러낸 기존 결함**: gate `KeycloakStateEntry` 에 Jackson 생성자가 없어 `fromJson` 이 한 번도 성공한 적이 없었다 — gate 자체 Keycloak 로그인 콜백은 state 소비 단계에서 항상 CSRF 오류였다(`@Jacksonized` 로 수정, 표준 OIDC_RP 경로는 이 코드를 지나지 않아 S6 끝-끝에서 안 보였다). 1차 커밋에서 잠금 임계치 블록이 누락된 것도 테스트가 잡았다.
- ✅ **설치본 정직화 (D3-b)**: V24 가 시드 기관 6개를 지운다(설치본은 기관 0개, 개발용은 `scripts/dev/seed-dev-agencies.sh` 가 관리 API 로 생성·API 키 1회 출력) · 코어 `application.yml` 플러그인 기본값 false, `idem-hub` 는 mock-auth 만 번들, NICE/OACX·AnyID 는 `editions/idem-kr-hub` runtimeOnly + `KrHubEditionEnvironmentPostProcessor` 가 코어 yml 보다 앞에서 true(환경변수로 끔) · CAST 진입점 = 프로파일 `protocol.endpoints.ssoEntry`(없으면 E-IDO-113), `sourceAgency` = `ido.platform.code`(`IDEM_PLATFORM_CODE`) · `NonOidcBrokerAdapter` 의 PASS·GPKI… 분기 → `ido.broker.nonoidc.providers` 설정(코어 빈 목록, KR 후처리기가 공급) · `ProviderRouter` KAKAO/NAVER 휴리스틱 → idp-hint 매핑 키 · `resolveAuthMethod` `_LOGIN` 접미 규칙 · 시간대 `Asia/Seoul` 고정 → `ido.zone`(컨테이너 TZ, 코어 기본 UTC) · `DeadLetterNotifier` 링크 설정화 · `login-flow.html`·`project_analysis.md`·기능명세서·`outputs/`·`infra/k8s`·`infra/minikube` 제거 · `GeneralizationGuardTest` 에 go.kr 토큰·플러그인 번들·V24 가드
- ✅ **CI 설치본 스모크 (D3-c)**: smoke-test 잡이 `install.env.example` 의 모든 필수 키를 1회용 값으로 채워 `compose.install.yml config` 를 검증하고, compose 와 같은 Keycloak 이미지·realm import·`KC_HOSTNAME_URL` 로 Keycloak 컨테이너를, registry·authz·gate·hub 는 compose 의 환경변수 이름 그대로 부트 jar 로 띄운 뒤 `scripts/ci/install-smoke.sh`(헬스 · Discovery issuer · 프로파일 PUT → client·secret 회전 · 로그인 화면 프록시/PKCE 사전검사 · Mock → registry 등록 · 이벤트 피드 · KR 404) → k6(실제 registry). 같은 스크립트로 로컬 실기동 전 항목 통과. **잡은 설치 결함**: registry 부팅 가드가 요구하는 `QIM_DI_SECRET`·`QIM_CI_AES_KEY_V1` 이 `compose.install.yml`·`install.env.example` 에 없어 설치본의 registry 가 기동을 거부했다 → 추가.
- ✅ 검증: common 418 · hub 525 · gate 124 · registry 223 · relay 10 · kr-hub 35 · sdk 40 · agent 250 · 로컬 설치본 스모크 7/7 (RegistryOutboxPoller 가 실제 registry 피드의 USER_UPDATED 를 처리, V24·authz V4 적용 확인)
- ⏭ **남긴 것**: `docker compose up --build`(이미지 빌드 포함) 자체는 PR 게이트 밖(main 의 docker-build 가 Dockerfile 을 검증) · 시드 기관의 "테스트 픽스처 이동" 은 스크립트로 대체(tenant-sample E2E 는 hub 를 목으로 써 시드가 필요 없었다) · `hub/qim/sp`(KR SP 수신기) 는 시스템 기본 시간대 사용 · S6 PR-3 항목(`IntegrationProtocol` SPI, idp-hint 프로파일화)은 그대로 이월
...
**진행 기록 (2026-09-25, S7 PR-1: 관리자 인증·인가)** — 구현 PR. 설계 근거는 `wiki/adr/ADR-015-admin-auth.md`, 운영·API 는 `docs/admin-auth.md`. `execution-plan.md` 결정 D3 는 **자체 계정 저장소** 로 닫혔다(Keycloak 은 ADR-014 로 숨겨져 관리 콘솔을 노출하지 않으므로 realm 위임은 그 콘솔을 TOE 에 되돌린다).
- ✅ **계정·역할·범위**: `ido.admin_user`(V25) — PBKDF2 비밀번호, 역할 `SYSTEM_ADMIN`/`POLICY_ADMIN`/`AUDITOR`, `tenant_code`(있으면 그 테넌트의 기관만: 목록 필터·단건 403·새 프로파일의 `service.tenant` 검사 — v0.5 결정 (4) 최소 구현), 상태·잠금·비밀번호 이력(3)·봉인된 TOTP 비밀. 부트스트랩: 관리자 0명이면 `IDEM_HUB_ADMIN_BOOTSTRAP_PASSWORD` 로 첫 `SYSTEM_ADMIN`(첫 로그인에서 변경 강제), `prod`/`stage` 에서 비면 기동 거부
- ✅ **인증**: 비밀번호 + TOTP(RFC 6238, `CryptoProvider.hmacSha1` 하나로만 SHA-1 을 연다 — `CryptoBoundaryGuardTest` 가 잡아 준 경계) 2단계, 첫 로그인에서 등록(`secret`/`otpauthUri` 1회 노출), 비밀은 `IDEM_HUB_ADMIN_SECRET_KEY` 로 AES-GCM 봉인. Redis 세션 유휴 15분·절대 8시간·동시 1, 쿠키 `idemAdminSid` HttpOnly·Secure·SameSite=Strict, 모든 쓰기(로그인 포함)에 `X-Requested-With` 필수(CSRF). 실패 5회 → 15분 잠금(423), 비밀번호 정책(10자·3종·사용자명·이력). 로그인·2단계 경로는 레이트리밋 대상
- ✅ **인가**: `AdminAuthFilter`(`OncePerRequestFilter`) + `AdminAuthorization` 매트릭스 — `/api/v1/admin/**`, `/actuator/**`(health·info·prometheus 제외), `DELETE /api/v1/handoff/*`. Spring Security 웹 체인 대신 필터 한 쌍(ADR-015 §3). 컨트롤러는 `X-Admin-Id` 대신 `AdminPrincipal` 인자(리졸버). **무인증 관리 엔드포인트 0** — 통합 테스트와 CI 스모크가 401/403 을 확인
- ✅ **감사**: 로그인 성공/실패/잠금·2단계 등록/실패·로그아웃·비밀번호 변경·관리자 CRUD/재설정/해제·**인가 거부·CSRF 거부** 가 `event_category=ADMIN`·`actor_type=ADMIN`(audit_log CHECK 확장), 기존 관리 행위 감사의 actor 는 인증된 사용자명. **감사 검색 API** `GET /api/v1/admin/audit`(기간·분류·사건·주체·기관·결과·상관ID, 테넌트 관리자는 자기 기관만)
- ✅ **관리자 관리 API** `/api/v1/admin/admins`(생성 → 임시 비밀번호 1회 표시, 역할/테넌트/상태 변경, 재설정, 해제, 2단계 초기화; 마지막 SYSTEM_ADMIN 강등·자기 강등 거부 E-IDO-136). 오류 코드 `E-IDO-130~139`
- ✅ **설치·CI·스크립트**: `install.env.example`·`compose.install.yml` 에 `IDEM_HUB_ADMIN_BOOTSTRAP_PASSWORD`·`IDEM_HUB_ADMIN_SECRET_KEY`(필수), `scripts/lib/admin-login.sh`(curl+python3 로 로그인·2단계 등록·첫 비밀번호 변경) 를 CI 설치본 스모크(②′ 무인증 401·CSRF 403·로그인, ⑧ 감사 조회 → 로그아웃)와 개발 시드 스크립트가 쓴다. `FailSecureBootGuard` 에 `ido.admin.cookie.secure`·`mfa.required`·`bootstrap.require-password-change`(운영 true) · `allow-derived-secret-key`(운영 false) 편입
...
**진행 기록 (2026-09-25, S9 PR-1: 개명 4b — 런타임 식별자)** — 구현 PR. 대응표·상태는 `docs/naming.md` §3.1, 업그레이드 절차는 `docs/sso-im-operations-manual.md`(개명 4b) · `docs/install.md` §2.
- ✅ **설정 키**: `ido.*`→`idem.hub.*`, `qsign.*`→`idem.gate.*`, `qim.*`→`idem.registry.*`, `authz.*`→`idem.authz.*`, `batch.*`→`idem.relay.*`, `agency-stub.*`→`idem.sample.*` + 모듈 안의 구 모듈 지칭(`qim→registry`, `q-authz→authz`, `qsign→gate`, `ido→hub`, `relay→jobs`, 호출자 `q-sign/outbox-relay→idem-gate/idem-relay`). yml 루트는 `idem.hub:` 같은 점 표기(기존 `idem:` 공통 루트와 공존)
- ✅ **환경변수**: `IDO_*/QSIGN_*/QIM_*/AUTHZ_*/BATCH_*`→`IDEM_HUB_*/IDEM_GATE_*/IDEM_REGISTRY_*/IDEM_AUTHZ_*/IDEM_RELAY_*`(+ 별칭 9개, S7 의 `IDEM_ADMIN_*`→`IDEM_HUB_ADMIN_*`). `DB_*`·`REDIS_*`·`KEYCLOAK_*`·벤더·설치본 공통 `IDEM_*` 는 유지. compose·install.env.example·CI·스크립트·Helm(values 키 `gate/registry/hub/relay/sample`, 리소스 `idem-hub-*`)·monitoring(`job="idem-hub"`, `idem_outbox_*`) 일괄
- ✅ **이름**: `spring.application.name` `idem-{gate,hub,registry,authz,relay,tenant-sample}`, Kafka client/group/토픽 `idem-*`/`idem.<모듈>.*`, 호출자·source_system `idem-hub`/`idem-gate`/`idem-registry`/`idem-authz`, Redis `ido:`→`idem:`·`qsign:`→`idem:gate:`·`fe:`→`idem:fe:`, Vault transit 기본 `idem-handoff-key`
- ✅ **호환 계층 (1 릴리스)**: `idem-common` `LegacyNames`(순수 대응 함수) + `LegacyNamesEnvironmentPostProcessor`(설정 파일 로딩 뒤) — 구 환경변수는 systemEnvironment 뒤에 새 이름으로 비추고, 구 설정 키는 그 소스 앞에 새 키로 비춘다(새 이름이 있으면 새 이름 우선). 기동 시 `[Idem 개명] 구 이름 N개…` WARN. 단위 테스트 3(대응표 30여 건·우선순위·멱등)
- ✅ **가드**: `NamingGuardTest` — 코어·에디션·플러그인 main, compose·env·Helm·스크립트·CI 에 구 설정 키(`${ido.`·`"ido.x"`·yml 루트)·구 환경변수·구 Redis 접두·구 앱/호출자 이름이 없다(스키마.테이블 `ido.outbox` 등과 오류 코드 상수 `IDO_TICKET_*` 는 제외 — 5단계·API 동결 대상)
- ✅ **그대로 둔 것(의도)**: DB `onepass`·스키마 `ido/qsign/qim/authz`·`spring.flyway` 스키마·JPA `schema="ido"`·SQL 문자열 · Keycloak realm `onepass`·client `q-sign-client`/`ido-client` · 감사 `source_system` CHECK 값 `ido-keycloak/…`(V3) · 오류 코드 enum 이름 `IDO_*`·코드 `E-IDO-1xx`(외부 계약) · `IDEM_HUB_BROKER_MODE` 값 `qsign`(모드 이름) · SDK/에이전트 `onepass.*`·`OnePass-Signature`(외부 계약, SDK 메이저에서) · 역사 문서(`docs/internal/architecture` 등)
- ✅ 검증: 단위 common 421 · hub 558 · gate 124 · registry 223 · authz 41 · relay 10 · tenant-sample 88 · kr-hub 35 · kr-registry 28 · 플러그인 54 · sdk 40 · agent 250 (개명 뒤 실패 3건 — 테스트 기대값의 구 이름 — 수정) · 통합 hub 46 · kr-hub 7(로컬 PG/Redis) · 실기동 설치본 스모크(Keycloak + registry·authz·gate·hub) **새 이름으로 ①~⑧ 전 항목** · **구 이름(IDO_*·QIM_*·QSIGN_*·AUTHZ_*·IDEM_ADMIN_*)만으로도 같은 항목 통과**(호환 계층 WARN 30개). 구 이름 실기동이 잡은 결함: 별칭을 `MapPropertySource` 로 넣으면 yml 자리표시자 없이 `@Value("${idem.hub.cast.private-key}")` 로 환경변수를 느슨하게 읽는 곳(CAST 키)이 못 받았다 → `SystemEnvironmentPropertySource` 로 교체(단위 테스트 추가)
- ⏭ **PR-2(5단계)**: DB·스키마·Keycloak realm/client · **agency→tenant 용어**(API 경로 `/admin/agencies`·프로파일 키·`X-Agency-Key`·이벤트 `qim.agency.events`)는 1.0 API 동결과 함께 판단(S9 PR-4) · Helm 은 텍스트 치환만(이 환경에 helm 없음) — 배포 전 `helm template`

**진행 기록 (2026-09-25, S9 PR-2: 개명 5단계 — DB·스키마·Keycloak)** — 구현 PR. 대응표는 `docs/naming.md` §3.1·§3.2, 업그레이드 절차는 `docs/sso-im-operations-manual.md`(개명 5단계) · `docs/install.md` §7, 스크립트 `scripts/upgrade/rename-db-1.0.sh`.
- ✅ **DB**: `onepass`→`idem`(이름·역할 기본값), 스키마 `ido`/`qsign`/`qim`/`authz`→`idem_hub`/`idem_gate`/`idem_registry`/`idem_authz`(`keycloak`·`agency_stub` 유지). Flyway 마이그레이션 파일의 스키마 접두를 **직접 고쳤다**(체크섬 변경) — 새 설치본은 새 이름으로 바로 만들고, 기존 설치본은 `idem-common` `LegacySchemaRename`(`FlywayMigrationStrategy` 자동 구성, relay 는 `BatchFlywayConfig` 에서 직접 호출)이 첫 기동에 `ALTER SCHEMA … RENAME`(구 스키마가 있고 새 스키마가 없을 때만) → `flyway.repair()` → `migrate()`. JPA `schema=`·SQL 문자열·`spring.flyway`·`currentSchema`·compose·CI·Helm·init-db.sql·pgadmin 일괄. hub V26: `auth_result.source_system` `ido-*`→`idem-hub-*`(CHECK 교체 + 기존 행 UPDATE), relay `source_system` `idem-relay`, CAST 폼 필드 `onepass_sso`→`idem_sso`
- ✅ **Keycloak**: realm `onepass`→`idem`, client `q-sign-client`/`ido-client`→`idem-gate`/`idem-hub`(`idem-provisioner`·`idem-session-manager` 그대로), realm 역할 `idem-user/admin`. `realm-export.json`·gate/hub `KeycloakProperties` 기본값·issuer 기본 `/realms/idem`·Back-Channel Logout 내부 client 목록·CI·compose·설치 스모크. 기존 설치본은 keycloak-data 볼륨을 지우고 realm 재import(사용자·세션은 Idem 이 registry 에 보관하므로 Keycloak 은 재생성 대상)
- ✅ **업그레이드 도구**: `scripts/upgrade/rename-db-1.0.sh [--keep-role] [--dry-run]` — DB 이름(연결이 남아 있으면 거부)·역할·스키마 rename, 멱등, dry-run 은 아직 안 옮긴 `onepass` 를 들여다본다. 스키마는 스크립트로 미리 옮겨도, 앱에 맡겨도 된다
- ✅ **가드**: `NamingGuardTest` 에 5단계 규칙 — JPA `schema="ido"`·`currentSchema=`·`.schemas("…")`·yml 스키마 루트·`ido.outbox` 류 스키마.테이블·`/realms/onepass`·`KEYCLOAK_REALM: onepass`·`"q-sign-client"`/`"ido-client"`·`onepass_sso`·`ido-(keycloak|nonoidc|adapter)`·`DB_NAME/POSTGRES_DB=onepass`·메트릭 `onepass.kms/outbox` 금지(`/db/migration/` 이력과 호환 계층 파일만 허용). 메트릭 `onepass.kms.healthy`→`idem.kms.healthy`, `onepass.outbox.*`→`idem.outbox.*`(alert_rules 동기)
- ✅ 검증: 단위 전 모듈(common 421·hub 558·gate 124·registry 223·authz 41·relay 10·tenant-sample 88·kr-hub 35·kr-registry 28·플러그인 54·sdk 40·agent 250 — 개명 뒤 실패 5건은 gate 테스트의 구 client id 기대값, 수정) · 통합 hub 46·kr-hub 7(로컬 PG, 새 DB → `idem_hub` 스키마 생성 확인) · **실기동 리허설 3종**(Keycloak 24.0.5 realm `idem` + registry·authz·gate·hub, 설치 스모크 ①~⑧ 전 항목): **A** 구 설치본 DB(`onepass`, 스키마 `ido/qsign/qim/authz`, V25 까지 적용, `ido-keycloak` 행 1건)에서 `ALTER DATABASE` 만 하고 기동 → 네 앱이 `[Idem 개명] 스키마 … → …` WARN 과 함께 스키마 rename·repair·V26 적용(`idem-hub-keycloak` 로 바뀜) · **C** 같은 상태에서 `rename-db-1.0.sh --keep-role` 로 미리 옮긴 뒤 기동(앱은 repair 만) · **B** `init-db.sql` 로 만든 새 DB. 리허설이 잡은 것: 관리자 계정이 이미 있는 DB 에서는 부트스트랩 비밀번호로 못 들어간다(정상 — 업그레이드 절차에 "기존 관리자 비밀번호 사용" 명시), dry-run 이 idem DB 가 없어 스키마 계획을 못 보여주던 것 수정
- ⏭ **남긴 것**: Helm 차트의 release/namespace/label 헬퍼 `onepass.*`·`mariadb` 블록(죽은 값) → S9 PR-3 Helm 재작성에서 · **agency→tenant 용어**(API 경로·프로파일 키·`X-Agency-Key`·이벤트 토픽)·오류 코드 `E-IDO-1xx`·SDK/에이전트 `onepass.*` 는 1.0 API 동결(S9 PR-4)에서 판단 · 호환 계층(`LegacyNames`·`LegacySchemaRename`)은 1 릴리스 뒤(1.1) 제거 · KR 에디션 코드 주석의 `onepass-fe/-be`(역사 서술)는 그대로

**진행 기록 (2026-09-26, S9 PR-3: 에디션 패키징)** — 구현 PR. Helm 은 `infra/helm/idem/README.md`, 입력값은 `docs/install-inputs.md`, 온보딩은 `docs/onboarding-guide.md`, 체크리스트는 `docs/requirements-checklist.md`, 회원 이관은 `scripts/kr-member-import/README.md`.
- ✅ **Helm 차트 재작성**: 종전 차트(OnePass 시절 — MariaDB·Kafka 전제·`onepass.*` 헬퍼·구 Secret 키)를 버리고 **단일 설치본 `compose.install.yml` 과 같은 계약**으로 다시 썼다 — 숨긴 Keycloak(realm import, production 모드 `start`, `KC_HOSTNAME_URL`=gate 공개 URL), gate·hub·registry·authz·console-admin(+KR 포털), PostgreSQL·Redis 는 바깥, Kafka 없음 기본(켜면 relay), pre-install Job 이 `files/init-db.sql` 로 스키마 생성, 앱 비밀 한 벌(`idem-app-secrets`, install.env 와 같은 키 이름)에서 Pod 마다 **필요한 키만** `secretKeyRef`. `global.edition=core|kr` → hub·registry 이미지 `<tag>-<edition>`, `values-kr.yaml`(KR 포털·플러그인 스위치). 구 `infra/helm/idem-hub` 차트·`values-dev/stage/prod` 삭제. `cd.yml` 은 `edition`·`values_file` 입력
- ✅ **이미지 변형**: `idem-hub/Dockerfile` 에 BuildKit 이름 붙은 컨텍스트 `vendor-libs`(Stage 0, 기본은 빈 디렉터리 `infra/docker/vendor-libs-empty`) → KR 이미지에 벤더 SDK 를 `--build-context vendor-libs=$HOME/.idem/vendor-libs` 로 넣는다(`-PvendorLibsDir`). compose 는 `additional_contexts` + `IDEM_VENDOR_LIBS_DIR`. CI `docker-build`(main) 는 hub·registry 를 core/kr 두 변형으로 민다(`:<sha>-core`·`:<sha>-kr`, core 는 접미 없는 태그도). PR 에는 `docker-build-check`(KR hub 를 push 없이 빌드) 와 `helm-lint`(lint · core/kr/Kafka 렌더 · kubeconform · `files/*` 사본이 `infra/docker` 원본과 같은지)
- ✅ **문서**: 설치 입력값 목록(비밀 20종의 생성 규칙·읽는 쪽·회전 영향, 비민감 설정의 compose↔Helm 대응, 기관에 전달하는 값), 기관 온보딩 가이드(작성 → 검증 → 시험 → 승인 4단계, 최소 `OIDC_RP` 프로파일 예, 검증 표, 시험 항목, 역할, 흔한 문제 — 상태는 `INACTIVE`/`ACTIVE` 둘뿐이고 승인 = SYSTEM_ADMIN 의 ACTIVE 전환), 요구사항 수용 체크리스트(플랜 §4 매트릭스 → 프로파일 키·확인 방법 + 설치본 쪽 수용 + **아직 안 되는 것**: SAML SP·SCIM·셀프서비스 신청·동의 카탈로그·Audit Sink·승인 역할 강제). `docs/README.md` 색인, `install.md` 포인터, 구판 `deployment/README.md` 표시, `CLAUDE.md`
...
- ✅ 검증: `helm lint`(core·kr) 0 오류, `helm template` core 1655줄·kr 1686줄을 파싱해 Deployment 7(kr)·env 항목 형식·프로브·Ingress 호스트 확인(이 환경에서 Go 로 helm 3.16 을 빌드해 씀) · 가드 `NamingGuardTest`·`GeneralizationGuardTest` 통과 · 이관 도구는 로컬 PG + `idem-kr-registry` jar 로 개인 2·기업 1·중복 CI 1 → `new=3 existing=1 biz=1`, 재실행 `new=0 existing=4 biz=exists`, `--resume` 전부 건너뜀, `--verify OK`, 코어 registry 에 BIZ 행은 404 오류로 기록·종료 코드 2(같은 PR 뒤 registry `GlobalExceptionHandler` 수정으로 500→404)
- ⏭ **남긴 것**: **실제 클러스터 배포는 못 해 봤다**(이 환경에 K8s 없음 — 첫 배포 때 README 의 확인 절차로 검증하고 기록) · Docker 빌드는 이 환경에 docker 가 없어 CI 의 `docker-build-check`(PR)·`docker-build`(main) 가 첫 검증 · 벤더 SDK 가 있는 KR 이미지 빌드는 사설 러너에서만 · Keycloak production 모드에서 `--import-realm` 의 재기동 동작(있으면 건너뜀)은 K8s 첫 배포 때 확인 · 실제 SMES 데이터 이관 리허설은 KR 고객 데이터가 있을 때
