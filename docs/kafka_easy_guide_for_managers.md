...
sequenceDiagram
    ...
    participant IDO as 다른 시스템들<br/>(IdO, Q-Sign 등)
    ...
    loop 0.5초마다 반복 확인
        ...
        KAFKA-->>IDO: ⑦ 메모 수신 → 각 시스템이 자기 역할대로 자동 처리
    end
```
...
### 📌 다이어그램을 읽기 전에 — 두 가지 흔한 오해

이 다이어그램을 처음 보면 두 가지 의문이 자연스럽게 생깁니다. 먼저 짚고 넘어가겠습니다.

#### ❓ 오해 1: "사용자가 Q-IM에 직접 접속하는 건가요?"

**아닙니다.** 실제 요청 경로는 다음과 같습니다.

```
실제 경로:    사용자 → (Nginx) → IdO(보안 검색대) → Q-IM(회원 시스템)
다이어그램:   사용자 ────────────────────────────→ Q-IM
```

이 다이어그램은 **"요청이 어디를 거쳐 가는가"가 아니라, "데이터가 어떻게 안전하게 퍼지는가"를 보여주는 것이 목적**입니다. Nginx와 IdO 경유 경로를 함께 그리면 핵심 메시지인 Outbox 패턴 설명이 묻혀버리기 때문에 의도적으로 생략했습니다. 요청 경로에 대한 상세 설명은 별도 문서(ext_api_proxy_guide)를 참고하시기 바랍니다.

#### ❓ 오해 2: "IdO를 알고 있는데, 왜 '다른 시스템들'이라고 표현했나요?"

**Q-IM은 Kafka 게시판에 메모를 붙이는 역할만 합니다. 누가 그 메모를 읽는지 알 필요도 없고, 알아서도 안 됩니다.**

주민센터 비유로 설명하면 이렇습니다:

> 주민센터에서 주소가 변경되면, 주민센터 담당자가 건강보험공단·국세청·운전면허센터에 각각 직접 전화를 해야 할까요?
> 아닙니다. 공지판에 공문 한 장을 붙여두면, 각 기관이 알아서 확인하고 처리합니다.
> **Q-IM이 하는 일이 정확히 이것입니다.**

실제로 Q-IM이 게시판에 남긴 "회원 정보 변경" 메모 하나를 **IdO와 Q-Sign이 동시에 읽어서 각자의 역할을 수행합니다.** 만약 다이어그램에 "IdO"라고만 명시하면, Q-Sign도 이 메모를 읽는다는 사실이 숨겨져 오히려 불완전한 그림이 됩니다. 이것이 "다른 시스템들"이라고 표현한 이유입니다.

이 두 경로의 차이를 한 줄로 정리하면 다음과 같습니다.

| 경로 | 방향 | IdO의 역할 |
|------|------|-----------|
| **요청 경로** | 사용자 → IdO → Q-IM | **창구** — 모든 요청을 받아 내부로 전달 |
| **결과 전파 경로** | Q-IM → Kafka → IdO, Q-Sign | **수신자 중 하나** — 게시판 공문을 읽고 자기 역할 처리 |

> IdO는 요청을 받는 창구이기도 하고, 결과 공문을 읽는 수신자이기도 합니다. 두 역할이 같은 시스템 안에 공존합니다.
...
> **시나리오를 읽기 전 참고**: 아래 다이어그램들에서 사용자와 각 시스템 간의 화살표는 실제로 IdO를 경유합니다. 시나리오별 핵심 흐름에 집중하기 위해 경유 경로는 생략하여 표현했습니다.
...
> **⑦ → ⑨ 흐름 보충 설명**: ⑦에서 IdO가 "인증 완료" 메모를 받는 것과, ⑨에서 사용자가 IdO에 이동 요청을 하는 것은 **별개의 독립적인 사건**입니다. ⑦은 Kafka 게시판을 통한 결과 전파(비동기)이고, ⑨는 사용자가 화면에서 버튼을 클릭하는 직접 요청(동기)입니다. IdO는 ⑦을 통해 인증 결과를 미리 캐시에 저장해두었기 때문에, ⑨ 요청이 들어왔을 때 DB를 다시 조회하지 않고 즉시 처리할 수 있습니다.
...
> **"기관에는 왜 알리지 않나요?"**: 계정 잠금은 OnePass 플랫폼 내부 상태입니다. 사용자가 이미 기관 서비스에 접속해 있다면 그 세션은 기관 서버가 관리하는 영역이므로 간섭하지 않습니다. OnePass가 할 수 있는 가장 빠른 조치는 **자체 화면(FE) 세션을 즉시 끊는 것**입니다. 기관 Webhook은 Handoff(기관 이동 티켓) 발급·취소 상황에만 발송됩니다.
...
> **⑨의 Outbox 패턴이 두 번 등장하는 이유**: Q-IM이 자신의 Outbox에 메모를 넣고(③), IdO가 그 메모를 받아 처리한 뒤 다시 자신의 provisioning_outbox에 68개 레코드를 넣습니다(⑨). Outbox 패턴이 중첩 적용된 것입니다. 이렇게 하면 "Q-IM → Kafka 전달"과 "IdO → 68개 기관 전달" 두 단계 모두 데이터 유실 없이 독립적으로 보장됩니다.
...
> **par 블록이 의미하는 것**: 다이어그램의 `par` 블록은 IdO와 Q-Sign이 **동일한 메모를 동시에 읽어서 각자 처리**한다는 뜻입니다. Q-IM은 게시판에 메모 하나만 붙였을 뿐이고, 누가 읽는지 관여하지 않습니다. 이것이 §2.2에서 설명한 "Q-IM은 수신자를 알 필요가 없다"는 원칙이 실제로 적용된 모습입니다.
