...
- 호환성: 기존 dev/local/test 환경은 `IDEM_HUB_KMS_ENABLED:false` 기본값으로 동일하게 작동.
...
- 신규 Kafka 이벤트 타입 `SIGNATURE_INVALID` 추가 (`idem.hub.handoff.events` 토픽, SIEM 연계용).
...
- **F4.3**: `@Value("${idem.hub.webhook.signing-secret:}")` 기본값 제거 + `@PostConstruct validateSigningSecret()` 부팅 검증 + `idem.hub.webhook.allow-empty-secret` escape hatch (테스트 전용). `application.yml` / `application-local.yml` / 테스트 yml 4개 모두 갱신.
...
- 호환성: 운영 환경은 `IDEM_HUB_WEBHOOK_SIGNING_SECRET` 환경변수 주입 의무화 (CrashLoopBackOff 발생 가능 — 사전 공지 필요). 프론트엔드는 `redirectUrl` 대신 `formHtml` 사용 필요 (마이그레이션 노트 §7.2).
...
- **F4.8 (내부 호출자 인증)**: 신규 `InternalCallerAuthInterceptor` + `idem.hub.internal.callers.{name}` properties. `POST /api/v1/fe-session` 및 `POST /api/v1/fe-session/conversion` 에 `X-Internal-Caller` + `X-Internal-Api-Key` 헤더 강제. `@PostConstruct` 부팅 검증 + `idem.hub.internal.allow-empty-callers` escape hatch (α-3 F4.3 패턴). `/check`, `/logout` 은 명시적 제외(최종 사용자 직접 호출). 운영: K8s Secret → 환경변수 (`IDEM_HUB_INTERNAL_API_KEY_GATE`, `IDEM_HUB_INTERNAL_API_KEY_RELAY`).
...
- **F2.1 (α-7, Keycloak client-secret)**: q-sign 측 `KeycloakProperties` 와 **ido 측 `KeycloakProperties` 두 곳 모두** `change-me` default 제거. 부팅 시 `@PostConstruct validateClientSecret()` 가 null/blank/placeholder/8자 미만 거부. `idem.gate.keycloak.allow-empty-client-secret` / `idem.hub.keycloak.allow-empty-client-secret` escape hatch (테스트 한정). ido test/local/integration-test yml 3개 갱신. α-3 F4.3 패턴 재사용.
- **F3.4 (α-3, DI HMAC secret)**: q-im `DiGenerationService` 의 `default-di-secret-change-in-production` default 제거. `@PostConstruct validateDiSecret()` 가 null/blank/placeholder/16자 미만 거부 (HMAC-SHA256 최소 보안 강도 요구). `idem.registry.crypto.di.allow-empty-secret` escape hatch. placeholder 목록에 과거 default 문자열 포함 → 재실수 차단.
...
- **호환성 영향 (중요)**: 운영 환경변수 의무화 — `IDEM_GATE_KEYCLOAK_CLIENT_SECRET`, `KEYCLOAK_CLIENT_SECRET` (ido), `IDEM_REGISTRY_DI_SECRET`. 미설정 또는 placeholder 주입 시 컨테이너 **CrashLoopBackOff** 발생 → 운영자가 즉시 인지 가능 (의도된 동작). 테스트/로컬 환경은 escape hatch 로 자동 통과.
...
- `idem.registry.crypto.ci.allow-empty-key` escape hatch — 단위 테스트는 `@BeforeEach` 의 reflection 주입으로 컨테이너 외부에서 인스턴스를 만들므로 영향 없음. `application-local.yml` 에 `allow-empty-key: true` 추가 (로컬 샌드박스 한정).
...
- **호환성 영향 (중요)**: 운영 환경변수 `IDEM_REGISTRY_CI_AES_KEY_V1` 의무화. 32바이트 무작위 Base64 키 (생성: `openssl rand -base64 32`). 미주입 시 `IllegalStateException` 으로 ApplicationContext 초기화 차단 → 컨테이너 CrashLoopBackOff. 기존 운영 환경에 이미 키가 주입되어 있다면 영향 없음. 만약 default 값에 의존하던 dev/stage 환경이 있다면 즉시 키 주입 필요.
...
- **이름 불일치 버그 동시 수정**: 정적 검증 중 `idem-hub/application.yml` 의 `idem.hub.crypto.handoff-aes-key` / `handoff-hmac-key` 는 어떤 Java 코드도 읽지 않는 dead config 였고, 실제 `KeyVersionRegistry` 는 yml 에 정의되지 않은 `idem.hub.ticket.aes-key` / `idem.hub.ticket.hmac-key` 를 읽고 있었다. 즉 환경변수 `IDEM_HUB_HANDOFF_AES_KEY` 가 주입되어도 매핑되지 않아 `@Value` default `AAAA...=` 가 운영에서 그대로 사용 — F3.3 가 차단하려던 위험 시나리오 자체. yml 의 `idem.hub.ticket:` 블록에 `aes-key: ${IDEM_HUB_HANDOFF_AES_KEY:}` / `hmac-key: ${IDEM_HUB_HANDOFF_HMAC_KEY:}` 추가로 환경변수 매핑 복원. `idem.hub.crypto.handoff-*-key` 는 deprecation 주석 처리.
- escape hatch: `idem.hub.ticket.allow-empty-fallback-keys` — 테스트/통합테스트 yml 2곳에 `true` 적용.
- **docker-compose 후속 정리**: γ-1/γ-2 에서 누락된 docker-compose.yml 의 placeholder default 4곳 정리 (`KEYCLOAK_CLIENT_SECRET:change-me`, `IDEM_HUB_WEBHOOK_SIGNING_SECRET:poc-webhook-secret-…`, `IDEM_HUB_HANDOFF_AES_KEY:AAAA…=`, `IDEM_HUB_HANDOFF_HMAC_KEY:AAAA…=`, `IDEM_GATE_KEYCLOAK_CLIENT_SECRET:change-me`). compose 의 `${VAR:?msg}` 문법으로 미주입 시 즉시 오류 종료 + 안내 메시지.
...
- **호환성 영향 (중요)**: 운영 환경변수 의무화 — `IDEM_HUB_HANDOFF_AES_KEY`, `IDEM_HUB_HANDOFF_HMAC_KEY` (둘 다 32바이트 Base64, `openssl rand -base64 32`). 미주입 시 컨테이너 **CrashLoopBackOff**. **이름 불일치 수정의 부수효과로 기존 운영 환경에 이미 `IDEM_HUB_HANDOFF_AES_KEY` 가 주입되어 있다면 이번 PR 이후 비로소 진짜 폴백 키로 동작**. 만약 기존에 placeholder `AAAA...=` 로 암호화된 Handoff Ticket 잔존분이 Redis/DB 어딘가에 남아있다면 새 키로 복호화 실패 → TTL 60초 한정이므로 운영 영향 미미. 단, 회복 시나리오 운영 노트에 명시 권장.
- 잔여 인증 본체 P0: F3.2 (findByIdentifierHash provider 별 조회) → γ-4 단독 PR 예정.
- 잔여 docker-compose placeholder (β-2 영역): `IDEM_HUB_INTERNAL_SIG_SECRET:change-me-…` (2곳), `IDEM_REGISTRY_INTERNAL_API_KEY:dev-…-change-me` (2곳) → β 후속 정리 후보로 별도 추적.
- 통합 PR: shipster→main, 본 PR 로 묶음.
