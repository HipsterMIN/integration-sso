package io.github.hipstermin.idem.hub.kr.auth.service;
...
import io.github.hipstermin.idem.hub.kr.auth.audit.KrAuthAuditService;
import io.github.hipstermin.idem.hub.kr.auth.client.IntegrationAuthClient;
import io.github.hipstermin.idem.hub.kr.auth.dto.*;
import io.github.hipstermin.idem.hub.kr.auth.port.ImApiOutPort;
import io.github.hipstermin.idem.hub.kr.auth.validation.MemberDivisionPolicy;
...
class AuthServiceTest {
    ...
    @Mock
    private KrAuthAuditService authAuditService;
    ...
    class CheckNiceCiTest {
        ...
        @Test
        @DisplayName("개인회원(A101) 정상 요청 — registry 기존 회원 조회 후 2000 (indvlMbrId 는 코어 계약 밖 → null)")
        void checkNiceCi_shouldReturnIndvlMbrIdForExistingA101Member() {
            ...
            // registry 코어는 SMES 개인 회원 ID 를 돌려주지 않는다 (S8-a: 종전에도 실제로는 null)
            assertThat(response.getIndvlMbrId()).isNull();
        ...
        @Test
        @DisplayName("기업회원(A102) registry 기존 회원 조회 후 2000 (cmpMbrId 는 코어 계약 밖 → null)")
        void checkNiceCi_shouldReturnCmpMbrIdForExistingA102Member() {
            ...
            assertThat(response.getResultCode()).isEqualTo("2000");
            assertThat(response.getResult()).isTrue();
            // registry 코어는 SMES 기업 회원 ID 를 돌려주지 않는다 (S8-a: 종전에도 실제로는 null)
            assertThat(response.getCmpMbrId()).isNull();
            assertThat(response.getResultMsg()).contains("기존");
        }
    }
