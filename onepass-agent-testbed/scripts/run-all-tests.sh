...
check_prerequisites() {
    ...
    # Agent JAR 확인
    local agent_jar
    agent_jar=$(find "$AGENT_JAR_DIR" -name "onepass-agent-*-all.jar" 2>/dev/null | sort -V | tail -1)
    if [[ -z "$agent_jar" ]]; then
        log_warn "Agent JAR를 찾을 수 없습니다: $AGENT_JAR_DIR"
        log_warn "다음 명령으로 빌드하세요:"
        log_warn "  cd $(dirname "$TESTBED_DIR") && ./gradlew :onepass-agent:agentJar"
        log_warn "  cp onepass-agent/build/libs/onepass-agent-*-all.jar $AGENT_JAR_DIR/"
