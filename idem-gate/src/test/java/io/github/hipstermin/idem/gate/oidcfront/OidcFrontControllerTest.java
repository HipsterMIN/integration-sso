...
class OidcFrontControllerTest {
    ...
    static final String ISSUER = "https://sso.example.org/realms/idem";
    ...
    void discovery() throws Exception {
        ...
        mvc.perform(get("/realms/idem/.well-known/openid-configuration")).andExpect(status().isOk())
                .andExpect(jsonPath("$.issuer").value(ISSUER));
    ...
    void authorize_preChecks() throws Exception {
        mvc.perform(get("/realms/idem/protocol/openid-connect/auth").param("client_id", "idem-gate")
        ...
        mvc.perform(get("/realms/idem/protocol/openid-connect/auth").param("client_id", "idem-svc-AG1")
        ...
        mvc.perform(get("/realms/idem/protocol/openid-connect/auth").param("client_id", "idem-svc-AG1").param("response_type", "code"))
    ...
    void authorize_forwardsAndRewritesLocation() throws Exception {
        kc.stubFor(WireMock.get(urlPathEqualTo("/realms/idem/protocol/openid-connect/auth"))
                .willReturn(aResponse().withStatus(302)
                        .withHeader("Location", "http://localhost:" + kc.port() + "/realms/idem/login-actions/authenticate?x=1")
                        .withHeader("Set-Cookie", "AUTH_SESSION_ID=abc; Path=/realms/idem/; HttpOnly")));
        mvc.perform(get("/realms/idem/protocol/openid-connect/auth").param("client_id", "idem-svc-AG1").param("response_type", "code")
                ...
                .andExpect(header().string("Location", "https://sso.example.org/realms/idem/login-actions/authenticate?x=1"))
    ...
    void token_hubDown() throws Exception {
        ...
        mvc.perform(post("/realms/idem/protocol/openid-connect/token").contentType(MediaType.APPLICATION_FORM_URLENCODED)
    ...
    void token_passthroughErrors() throws Exception {
        ...
        mvc.perform(post("/realms/idem/protocol/openid-connect/token").contentType(MediaType.APPLICATION_FORM_URLENCODED)
                        .content("grant_type=authorization_code&client_id=idem-hub&code=c1"))
    ...
    void userInfo_enriched() throws Exception {
        kc.stubFor(WireMock.get(urlEqualTo("/realms/idem/protocol/openid-connect/userinfo"))
                .willReturn(aResponse().withStatus(200).withHeader("Content-Type", "application/json")
        ...
        mvc.perform(get("/realms/idem/protocol/openid-connect/userinfo").header("Authorization", "Bearer " + bearer))
    ...
    void passthrough() throws Exception {
        kc.stubFor(WireMock.post(urlPathEqualTo("/realms/idem/login-actions/authenticate"))
                .willReturn(aResponse().withStatus(302).withHeader("Location", "https://rp/cb?code=xyz&state=s")));
        mvc.perform(post("/realms/idem/login-actions/authenticate").param("session_code", "sc").contentType(MediaType.APPLICATION_FORM_URLENCODED)
        ...
        mvc.perform(post("/realms/idem/clients-registrations/openid-connect")).andExpect(status().isNotFound());

        kc.stop();
        try {
            mvc.perform(get("/realms/idem/protocol/openid-connect/certs")).andExpect(status().isBadGateway())
                    .andExpect(jsonPath("$.error").value("temporarily_unavailable"));
        } finally {
            kc.start();
        }
    ...
    void rpInitiatedLogout_cleansIdemSideThenForwards() throws Exception {
        ...
        mvc.perform(get("/realms/idem/protocol/openid-connect/logout").param("id_token_hint", "hint.p.s")
