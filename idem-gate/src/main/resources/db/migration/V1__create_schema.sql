...
--    연동 메타를 관리한다.
-- ──────────────────────────────────────────────────────────────
CREATE TABLE idem_gate.idp_provider (
...
COMMENT ON TABLE  idem_gate.idp_provider                IS '외부 인증 공급자(IdP) 등록 및 연동 메타';
COMMENT ON COLUMN idem_gate.idp_provider.provider_code  IS '고유 식별 코드 (KAKAO_OIDC, PASS, GPKI 등)';
COMMENT ON COLUMN idem_gate.idp_provider.auth_level      IS '해당 공급자가 보증하는 인증 수준 (L1/L2/L3)';
COMMENT ON COLUMN idem_gate.idp_provider.client_secret_ref IS 'Secret Manager 참조 경로 – 평문 저장 금지';
...
-- ──────────────────────────────────────────────────────────────
CREATE TABLE idem_gate.auth_result (
    ...
    CONSTRAINT fk_auth_result_provider
        FOREIGN KEY (provider_code) REFERENCES idem_gate.idp_provider(provider_code),
...
CREATE INDEX idx_auth_result_identifier_hash ON idem_gate.auth_result (identifier_hash);
CREATE INDEX idx_auth_result_correlation_id  ON idem_gate.auth_result (correlation_id);
CREATE INDEX idx_auth_result_provider_tx     ON idem_gate.auth_result (provider_code, provider_tx_id)
    WHERE provider_tx_id IS NOT NULL;
CREATE INDEX idx_auth_result_created_at      ON idem_gate.auth_result (created_at DESC);

COMMENT ON TABLE  idem_gate.auth_result                     IS '§9 Q-Sign 발급 인증 결과 SoR';
COMMENT ON COLUMN idem_gate.auth_result.identifier_hash     IS 'SHA-256(CI) 또는 SHA-256(idToken.sub) – 역방향 복호화 불가';
COMMENT ON COLUMN idem_gate.auth_result.internal_signature  IS '내부 위변조 검증용 HMAC-SHA256 서명';
COMMENT ON COLUMN idem_gate.auth_result.claims              IS 'IdP 반환 클레임 – 이름/연락처 등 PII 마스킹 필수';
...
-- ──────────────────────────────────────────────────────────────
CREATE TABLE idem_gate.auth_lock (
...
CREATE INDEX idx_auth_lock_locked ON idem_gate.auth_lock (locked, unlock_at)
    WHERE locked = TRUE;

COMMENT ON TABLE  idem_gate.auth_lock           IS '§9.6 인증 수단별 잠금·재시도 카운터 SoR';
COMMENT ON COLUMN idem_gate.auth_lock.lock_key  IS '{identifierHash}:{providerCode} 복합 키';

-- ──────────────────────────────────────────────────────────────
-- 4. Transactional Outbox (Q-Sign → Kafka)
--    §9.3 인증 이벤트를 Kafka idem_gate.auth.events 로 발행.
--    Outbox Relay 배치가 PENDING 레코드를 발행 후 PUBLISHED 처리.
-- ──────────────────────────────────────────────────────────────
CREATE TABLE idem_gate.outbox (
    ...
    topic                VARCHAR(200)  NOT NULL DEFAULT 'idem_gate.auth.events',
...
CREATE INDEX idx_qsign_outbox_status     ON idem_gate.outbox (status, created_at)
    WHERE status = 'PENDING';
CREATE INDEX idx_qsign_outbox_aggregate  ON idem_gate.outbox (aggregate_id);

COMMENT ON TABLE  idem_gate.outbox               IS '§9.3 Transactional Outbox – idem_gate.auth.events 발행용';
COMMENT ON COLUMN idem_gate.outbox.partition_key IS 'Kafka 파티션 키 = identifierHash';
COMMENT ON COLUMN idem_gate.outbox.status        IS 'PENDING → PUBLISHED | FAILED';

-- ──────────────────────────────────────────────────────────────
-- 5. Flyway 메타 전용 샘플 데이터 (IdP 공급자 초기값)
-- ──────────────────────────────────────────────────────────────
INSERT INTO idem_gate.idp_provider
