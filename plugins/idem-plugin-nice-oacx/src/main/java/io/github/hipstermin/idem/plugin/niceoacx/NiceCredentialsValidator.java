 ...
 * {@code idem.hub.auth.allow-missing-credentials=true} 면 경고 후 계속(해당 제공자는 호출 시 실패).
 */
@Slf4j
public final class NiceCredentialsValidator {
    ...
    public static void validate(NiceProperties nice, OacxProperties oacx, boolean oacxAvailable, boolean allowMissing) {
        ...
        if (allowMissing) {
            log.warn("[NicePlugin] 운영 프로파일이나 본인인증 자격증명 미설정: {} — idem.hub.auth.allow-missing-credentials=true 로 기동 계속(해당 본인인증 비활성).", missing);
            return;
        }
        throw new IllegalStateException("운영(prod) 기동 차단 — 본인인증 자격증명 미설정: " + missing
                + ". 환경변수를 설정하거나, 본인인증을 쓰지 않는 배포라면 idem.hub.auth.allow-missing-credentials=true 로 명시적 비활성화하세요.");
    }
}
