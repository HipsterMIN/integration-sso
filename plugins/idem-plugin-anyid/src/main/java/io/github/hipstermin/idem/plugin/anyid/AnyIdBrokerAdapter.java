 ...
 */
@Slf4j
public class AnyIdBrokerAdapter implements DirectBrokerAdapter {

    /** {@code provider_config.broker_mode} 값. */
    public static final String ID = "anyid";

    private final AnyIdProperties anyIdProperties;
    private final ObjectMapper    objectMapper;
    private final RestTemplate    restTemplate;

    public AnyIdBrokerAdapter(AnyIdProperties anyIdProperties, ObjectMapper objectMapper) {
        this(anyIdProperties, objectMapper, defaultRestTemplate(anyIdProperties));
    }

    AnyIdBrokerAdapter(AnyIdProperties anyIdProperties, ObjectMapper objectMapper, RestTemplate restTemplate) {
    ...
    private static RestTemplate defaultRestTemplate(AnyIdProperties props) {
        AnyIdProperties.Http http = props.getHttp();
    ...
    /**
     * Any-ID 인증수단 코드 휴리스틱 (DB {@code provider_config.broker_mode} 가 없을 때):
     * {@code ANYID_*} 접두 또는 MOBILE_ID / EASY_SIGN / JOINT_CERT / FINANCIAL_CERT / PRIVATE_ID.
     */
    @Override
    public boolean supports(String providerCode) {
        if (providerCode == null) return false;
        String code = providerCode.toUpperCase().replace("-", "_");
        return code.startsWith("ANYID_")
                || "MOBILE_ID".equals(code)
                || "EASY_SIGN".equals(code)
                || "JOINT_CERT".equals(code)
                || "FINANCIAL_CERT".equals(code)
                || "PRIVATE_ID".equals(code);
    }
    ...
    /**
     * Any-ID Authorization URL 생성 — 코어 BrokerService 가 {@link DirectBrokerAdapter} 로 호출
     *
     * <p>provider → Any-ID 인증 수단 URL 매핑:
     * <pre>
