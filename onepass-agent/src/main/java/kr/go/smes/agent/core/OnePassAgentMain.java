 ...
 * byte-buddy shading({@code net.bytebuddy} → {@code kr.go.smes.agent.shaded.bytebuddy})으로
    ...
    private static void startHealthCheckThread(final AgentConfig config, final PrintStream log) {
        Thread healthThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    kr.go.smes.agent.http.OnePassHttpClient client =
                            new kr.go.smes.agent.http.OnePassHttpClient(config);
                    kr.go.smes.agent.http.OnePassHttpClient.HttpResponse resp = client.healthCheck();
                    if (resp.isSuccess()) {
