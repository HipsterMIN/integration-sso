/**
 * outbox-relay-batch — Transactional Outbox 분산 릴레이 배치 서비스
 *
 * ┌─────────────────────────────────────────────────────────────────────────┐
 * │  설계 배경                                                               │
 ...
 * │                                                                         │
 * │  DB 접근 전략:                                                           │
 * │  - ido(PostgreSQL) / q-im(MariaDB) / q-sign(PostgreSQL) 각각 독립 DS    │
 * │  - Spring Boot 멀티 DataSource 설정                                      │
 * │  - Flyway: ido DS만 마이그레이션 (shedlock 테이블 V19)                   │
 * └─────────────────────────────────────────────────────────────────────────┘
 */
...
dependencies {
    ...
    implementation("org.springframework.boot:spring-boot-starter-data-jpa")      // JPA (q-sign 엔티티)
    implementation("org.springframework.boot:spring-boot-starter-data-redis")    // ShedLock Redis Provider

    // ── JDBC (ido/q-im 직접 JDBC 접근 — JSONB 컨트롤) ─────────────────────────
    implementation("org.springframework.boot:spring-boot-starter-jdbc")

    // ── DB 드라이버 ──────────────────────────────────────────────────────────
    implementation("org.postgresql:postgresql")                                   // ido, q-sign
    implementation("org.mariadb.jdbc:mariadb-java-client")                       // q-im
    ...
    // ── Flyway (shedlock 테이블 마이그레이션 — ido PostgreSQL) ─────────────────
