...
import { clearAll, markAuthed, saveCiToken } from './services';
...
// PUB260513 find_password_member.html — 비밀번호 찾기 step1 (인증 수단 선택)
// FindId 와 동일한 4 카드 구성 (개인인증서 / 개인 간편인증서 / 휴대폰 / Any-ID)
function FindPassword(): JSX.Element {
	...
	const issueCiToken = useCallback(async (ci: string, authInfo?: { name?: string; birthDate?: string; gender?: 'M' | 'F'; phone?: string }): Promise<void> => {
		...
		markAuthed();
		history.push(ROUTES.FIND_PASSWORD_NEW);
	}, []);
	...
	const handlePhoneAuthSuccess = useCallback(
		(result: NicePhoneAuthResult): void => {
			...
			issueCiToken(ci, {
				...
				console.error('[FindPassword] 휴대폰인증 ciToken 발급 실패:', err);
				setFailedMessage('');
				setFailedModal(true);
			});
		},
	...
	return (
		<>
			<main id="main-content" className="container sub find_password step1 member">
					...
					<div className="form-container">
						<div className="form-wrap white-wrap">
							<div className="title-box">
								...
								<p className="text">
									아래의 방식으로 인증하시고 새로운 비밀번호를 입력해주세요.
								</p>
							</div>
			...
			<Modal
				id="modal_find_password_dev_notice"
			...
			<Modal
				id="modal_find_password_auth_failed"
				isOpen={failedModal}
				onClose={(): void => {
