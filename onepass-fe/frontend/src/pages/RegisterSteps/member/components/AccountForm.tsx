...
function AccountForm({
	...
	// 검증 상태 변경 시 부모에 전달
	const formValid = isLoginIdValid(data.loginId) && getPasswordStrength(data.password).valid && !!passwordConfirm && data.password === passwordConfirm;

	useEffect(() => {
		onVerificationChange?.({
			validateOk: isBusiness ? validateStatus === 'ok' : true,
			duplicateOk: duplicateStatus === 'ok',
			formValid,
		});
	}, [isBusiness, validateStatus, duplicateStatus, onVerificationChange, formValid]);
	...
	// 로그인 ID 중복확인 (기업 전용)
	const handleLoginIdDuplicate = async (): Promise<void> => {
		const value = data.loginId;
		if (!value) return;
		if (!isLoginIdValid(value)) {
			setLoginIdDupStatus('error');
			setLoginIdDupMessage('아이디 형식을 확인해 주세요. (5~20자, 영문 소문자/숫자/_, .)');
			return;
		}
				...
				<>
					...
					<div className="input-wrap width50">
						<label htmlFor="login_id">
							로그인 ID<span className="essential">필수</span>
							<small className="info-text">
								5~20자의 영문 소문자, 숫자와 특수기호 (_), (.)만 사용
							</small>
						</label>
						<div className="input-box">
							<input
								...
								aria-required="true"
								autoComplete="username"
								aria-describedby="login_id_hint"
							/>
							<button
								type="button"
								className="btn point"
								aria-label="중복확인"
								onClick={handleLoginIdDuplicate}
						...
						<div className="input-hint-box">
							{loginIdFeedback && loginIdDupStatus === 'idle' && (
								<p className="invalid" id="login_id_hint">{loginIdFeedback.msg}</p>
							)}
					...
					<div className="input-wrap width50">
						...
						{passwordFeedback && (
							<div className="input-hint-box">
								<p className="invalid" id="biz_pw_hint">{passwordFeedback.msg}</p>
							</div>
						)}
					</div>
					<div className="input-wrap width50">
						<label htmlFor="biz_password_check">
							비밀번호 확인<span className="essential">필수</span>
						</label>
						<div className="input-box">
							<input
								type={showPasswordConfirm ? 'text' : 'password'}
								id="biz_password_check"
								...
								aria-required="true"
								autoComplete="new-password"
								aria-describedby="biz_pw_check_error"
							/>
							<button
								type="button"
								className="btn large icon pw-check"
						...
						{passwordConfirm && data.password !== passwordConfirm && (
							<div className="input-hint-box">
								<p className="invalid" id="biz_pw_check_error">비밀번호가 일치하지 않습니다.</p>
							</div>
						)}
					</div>
					<div className="input-wrap width50">
