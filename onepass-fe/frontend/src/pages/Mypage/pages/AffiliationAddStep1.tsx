...
function BusinessAuth({ onNext }: { onNext: string }): JSX.Element {
	...
	return (
		<>
			...
			<Modal
				id="modal_affiliation_add_dev_notice"
			...
			<Modal
				id="modal_affiliation_add_auth_failed"
				isOpen={failedModal}
...
function MemberAuth({ onNext }: { onNext: string }): JSX.Element {
	...
	return (
		<>
			...
			<Modal
				id="modal_affiliation_add_member_dev_notice"
				isOpen={devNoticeModal}
				onClose={(): void => setDevNoticeModal(false)}
				topText="안내"
				title="서비스 준비 중"
			...
			<Modal
				id="modal_affiliation_add_member_auth_failed"
				isOpen={failedModal}
				onClose={(): void => setFailedModal(false)}
				topText="안내"
...
function AffiliationAddStep1(): JSX.Element {
	const memberType = useMypageType();
	const isBusiness = memberType === 'business';
	const nextRoute = getMypageRoute(memberType, 'AFFILIATION_ADD_STEP2');
...
export default AffiliationAddStep1;
