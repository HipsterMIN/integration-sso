...
COMMENT ON COLUMN idem_hub.agency_meta.profile
    IS 'Tenant Profile (tenant-profile.v{n}.schema.json). 기관 설정의 단일 원천 — 낱개 컬럼은 이 문서의 투영';
COMMENT ON COLUMN idem_hub.agency_meta.profile_schema_version
...
CREATE INDEX IF NOT EXISTS idx_agency_meta_profile_type
    ON idem_hub.agency_meta ((profile -> 'protocol' ->> 'type'));
