...
-- ──────────────────────────────────────────────────────────────
-- 1. idem_hub.provider_config에 provider_type 컬럼 추가 (§18.5.4)
...
CREATE INDEX IF NOT EXISTS idx_broker_audit_correlation ON idem_hub.broker_audit_log (correlation_id);
CREATE INDEX IF NOT EXISTS idx_broker_audit_provider    ON idem_hub.broker_audit_log (provider_code, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_broker_audit_created     ON idem_hub.broker_audit_log (created_at DESC);
CREATE INDEX IF NOT EXISTS idx_broker_audit_error       ON idem_hub.broker_audit_log (error_code, created_at DESC)
    WHERE error_code IS NOT NULL;

COMMENT ON TABLE  idem_hub.broker_audit_log                IS '§18.5.5 브로커 구간 전 주기 감사 로그';
COMMENT ON COLUMN idem_hub.broker_audit_log.correlation_id IS '§17.5 X-Correlation-Id — 전 구간 추적 키';
COMMENT ON COLUMN idem_hub.broker_audit_log.provider_tx_id IS 'IDP 측 트랜잭션 ID (Keycloak sub / PASS tx 등)';
COMMENT ON COLUMN idem_hub.broker_audit_log.error_code     IS 'PlatformErrorCode 코드 문자열 (E-IDP-401 등)';
...
-- ──────────────────────────────────────────────────────────────
-- 참고: 이 파일은 ido 스키마용이므로 idem_gate.processed_event 는
