...
-- =============================================================================
--
-- 1. idem_hub.tenant — Tenant(Realm): 운영기관·사용자 디렉터리 단위. 설치본은 최소 1개(DEFAULT).
...
-- 1. Tenant
CREATE TABLE IF NOT EXISTS idem_hub.tenant (
...
COMMENT ON TABLE  idem_hub.tenant IS 'Tenant(Realm) — 운영기관·사용자 디렉터리 단위. Service(agency_meta)는 Tenant 에 속한다';
COMMENT ON COLUMN idem_hub.tenant.tenant_code IS 'Tenant 코드 — registry qim_user.tenant_code 와 같은 값';

INSERT INTO idem_hub.tenant (tenant_code, name, status)
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_agency_meta_tenant') THEN
        ALTER TABLE idem_hub.agency_meta
            ...
            REFERENCES idem_hub.tenant (tenant_code);
    END IF;
END $$;
CREATE INDEX IF NOT EXISTS idx_agency_meta_tenant ON idem_hub.agency_meta (tenant_code);
COMMENT ON COLUMN idem_hub.agency_meta.tenant_code IS '소속 Tenant(Realm). S4b';
...
-- 4. 전 기관 프로비저닝 제거
DROP TABLE IF EXISTS idem_hub.provisioning_outbox;
DELETE FROM idem_hub.agency_endpoint_registry WHERE endpoint_type = 'PROVISIONING';
