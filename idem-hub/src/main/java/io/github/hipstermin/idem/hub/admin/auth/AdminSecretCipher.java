...
/**
 * 관리자 TOTP 비밀 봉인 — AES-256-GCM, 키는 {@code idem.hub.admin.secret-key}({@code IDEM_HUB_ADMIN_SECRET_KEY}).
 * 키가 비면 로컬·테스트에 한해 Handoff AES 키에서 HMAC 파생한다({@code idem.hub.admin.allow-derived-secret-key=true} 필요).
 */
@Slf4j
@Component
public class AdminSecretCipher {
    ...
    void init() {
        ...
        if (configured != null && !configured.isBlank()) {
            ...
            if (key.length != 32) throw new IllegalStateException("idem.hub.admin.secret-key 는 base64 32바이트여야 합니다 (현재 " + key.length + "바이트)");
            return;
        }
        if (!props.isAllowDerivedSecretKey()) {
            throw new IllegalStateException("IDEM_HUB_ADMIN_SECRET_KEY(idem.hub.admin.secret-key) 가 비어 있습니다 — 관리자 TOTP 비밀을 봉인할 수 없습니다. "
                    + "openssl rand -base64 32 로 만들어 주입하십시오. 로컬·테스트에서만 idem.hub.admin.allow-derived-secret-key=true");
        }
        String handoffKey = environment.getProperty("idem.hub.ticket.aes-key", "");
        ...
        log.warn("[AdminSecretCipher] idem.hub.admin.secret-key 미설정 — 파생 키 사용 (로컬·테스트 한정, 운영 금지)");
    }
