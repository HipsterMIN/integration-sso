 ...
 *
 * <h2>환경변수 독립성 원칙</h2>
 * <p>F-01(IP RL)의 {@code IDEM_HUB_AUTH_RL_ENABLED}와 F-02(기관 RL)의 {@code IDEM_HUB_RATE_LIMIT_ENABLED}는
...
public class FeatureFlags {
    ...
    /** /api/v1/auth/** IP 단위 TPS/분당/일별 제한. 독립 환경변수 IDEM_HUB_AUTH_RL_ENABLED */
    @Value("${idem.hub.auth.rate-limit.enabled:${IDEM_HUB_AUTH_RL_ENABLED:true}}")
    private boolean authRateLimit;

    // ── F-02: 기관별 Rate Limiting ──────────────────────────────────────────
    /** 기관(agencyCode) 단위 TPS/일별 제한. 환경변수 IDEM_HUB_RATE_LIMIT_ENABLED */
    @Value("${idem.hub.rate-limit.enabled:${IDEM_HUB_RATE_LIMIT_ENABLED:true}}")
    ...
    // ── F-03: 감사 로그 Kafka 발행 ──────────────────────────────────────────
    /** platform.audit.log 토픽 비동기 발행. 환경변수 IDEM_HUB_AUDIT_KAFKA_ENABLED */
    ...
    // ── F-05: OTel 분산 추적 AOP ────────────────────────────────────────────
    /** AuthTracingAspect 빈 등록 여부. 환경변수 IDEM_HUB_AUTH_TRACING_ENABLED */
    @Value("${idem.hub.tracing.auth-aspect-enabled:${IDEM_HUB_AUTH_TRACING_ENABLED:true}}")
    private boolean authTracing;

    // ── F-08: Redisson 분산 락 ──────────────────────────────────────────────
    /** RedissonClient 빈 등록 여부. 환경변수 IDEM_HUB_REDISSON_ENABLED
     * false 시 NoOpRedissonConfig의 NoOp 프록시 사용 */
    @Value("${idem.hub.redisson.enabled:${IDEM_HUB_REDISSON_ENABLED:true}}")
    private boolean redissonLock;

    // ── F-10: 보안 응답 헤더 필터 ────────────────────────────────────────────
    /** SecurityHeadersFilter 빈 등록 여부. 환경변수 IDEM_HUB_SECURITY_HEADERS_ENABLED */
    @Value("${idem.hub.security-headers.enabled:${IDEM_HUB_SECURITY_HEADERS_ENABLED:true}}")
    private boolean securityHeaders;

    // ── F-11: 개인정보 파기 스케줄러 ────────────────────────────────────────
    /** 파기 스케줄러 활성 여부. 환경변수 IDEM_HUB_RETENTION_ENABLED (기본 false — 안전)
    ...
    /** 파기 dry-run 모드. 환경변수 IDEM_HUB_RETENTION_DRY_RUN (기본 true — 안전)
    ...
    /** AES 키 90일 주기 로테이션. 환경변수 IDEM_HUB_CRYPTO_ROTATION_ENABLED */
    @Value("${idem.hub.crypto.rotation-enabled:${IDEM_HUB_CRYPTO_ROTATION_ENABLED:true}}")
    private boolean cryptoRotation;

    // ── F-13: IdO Outbox Relay ───────────────────────────────────────────────
    /** idem.hub.outbox PENDING 이벤트 Kafka 재발행. 환경변수 IDEM_HUB_OUTBOX_RELAY_ENABLED */
    ...
    /** webhook_dispatch_outbox PENDING 기관 HTTP 발송. 환경변수 IDEM_HUB_WEBHOOK_RELAY_ENABLED */
    @Value("${idem.hub.webhook.relay-enabled:${IDEM_HUB_WEBHOOK_RELAY_ENABLED:true}}")
    private boolean webhookRelay;

    // ── F-18: SP 수신 감사 로그 ──────────────────────────────────────────────
    /** Q-IM SP 수신 API 감사 로그. 환경변수 IDEM_HUB_REGISTRY_RECEIVER_AUDIT */
    @Value("${idem.hub.registry.receiver-audit-enabled:${IDEM_HUB_REGISTRY_RECEIVER_AUDIT:true}}")
     ...
     * true 전환 전 {@code IDEM_HUB_PROVISIONING_DRY_RUN=true}로 먼저 로그 관찰 필수.
     *
     * <p><b>Phase 1 기본값: false</b> (Phase 2에서 true로 전환)
     *
     * @see docs/features/F-20-provisioning.md
     */
    @Value("${idem.hub.provisioning.enabled:${IDEM_HUB_PROVISIONING_ENABLED:false}}")
     ...
     */
    @Value("${idem.hub.provisioning.relay-enabled:${IDEM_HUB_PROVISIONING_RELAY_ENABLED:false}}")
     ...
     * @see docs/features/F-20-provisioning.md
     */
    @Value("${idem.hub.provisioning.dry-run:${IDEM_HUB_PROVISIONING_DRY_RUN:true}}")
     ...
     */
    @Value("${idem.hub.gateway.inbound-enabled:${IDEM_HUB_GATEWAY_INBOUND_ENABLED:false}}")
     ...
     */
    @Value("${idem.hub.gateway.outbound-enabled:${IDEM_HUB_GATEWAY_OUTBOUND_ENABLED:false}}")
     ...
     */
    @Value("${idem.hub.gateway.idempotency-enabled:${IDEM_HUB_GATEWAY_IDEMPOTENCY_ENABLED:true}}")
     ...
     */
    @Value("${idem.hub.gateway.hmac-sig-required:${IDEM_HUB_HMAC_SIG_REQUIRED:false}}")
     ...
     */
    @Value("${idem.hub.gateway.agency-key-audit-log:${IDEM_HUB_AGENCY_KEY_AUDIT_LOG:true}}")
     ...
     * [FeatureFlags]   F-01 authRateLimit       = true   (IDEM_HUB_AUTH_RL_ENABLED)
     * ...
     * [FeatureFlags] ── Phase-Gate 신규 기능 (Sprint 14~16) ──
     * [FeatureFlags]   F-20 provisioning        = false  (IDEM_HUB_PROVISIONING_ENABLED)  ← Phase 1
     * </pre>
     */
    @PostConstruct
    public void logFeatureFlags() {
        ...
        // ── 기존 기능 ──
        log.info("[FeatureFlags]   F-01 authRateLimit       = {}  (IDEM_HUB_AUTH_RL_ENABLED)", fmt(authRateLimit));
        log.info("[FeatureFlags]   F-02 agencyRateLimit     = {}  (IDEM_HUB_RATE_LIMIT_ENABLED)", fmt(agencyRateLimit));
        log.info("[FeatureFlags]   F-31 kafkaEnabled        = {}  (IDEM_KAFKA_ENABLED) — false 면 F-03·F-30 은 강제 OFF, F-13 은 프로세스 내 배달", fmt(kafkaEnabled));
        log.info("[FeatureFlags]   F-03 auditKafka          = {}  (IDEM_HUB_AUDIT_KAFKA_ENABLED)", fmt(auditKafka));
        log.info("[FeatureFlags]   F-04 auditDb             = {}  (IDEM_HUB_AUDIT_DB_ENABLED)", fmt(auditDb));
        log.info("[FeatureFlags]   F-05 authTracing         = {}  (IDEM_HUB_AUTH_TRACING_ENABLED)", fmt(authTracing));
        log.info("[FeatureFlags]   F-08 redissonLock        = {}  (IDEM_HUB_REDISSON_ENABLED)", fmt(redissonLock));
        log.info("[FeatureFlags]   F-10 securityHeaders     = {}  (IDEM_HUB_SECURITY_HEADERS_ENABLED)", fmt(securityHeaders));
        log.info("[FeatureFlags]   F-11 retentionJob        = {}  (IDEM_HUB_RETENTION_ENABLED)", fmt(retentionJob));
        log.info("[FeatureFlags]   F-11b retentionDryRun   = {}  (IDEM_HUB_RETENTION_DRY_RUN)", fmt(retentionDryRun));
        log.info("[FeatureFlags]   F-12 cryptoRotation      = {}  (IDEM_HUB_CRYPTO_ROTATION_ENABLED)", fmt(cryptoRotation));
        log.info("[FeatureFlags]   F-13 outboxRelay         = {}  (IDEM_HUB_OUTBOX_RELAY_ENABLED)", fmt(outboxRelay));
        log.info("[FeatureFlags]   F-14 webhookRelay        = {}  (IDEM_HUB_WEBHOOK_RELAY_ENABLED)", fmt(webhookRelay));
        log.info("[FeatureFlags]   F-18 spReceiverAudit     = {}  (IDEM_HUB_REGISTRY_RECEIVER_AUDIT)", fmt(spReceiverAudit));

        // ── Sprint 14~16 신규 기능 (Phase-Gate 관리) ──
        log.info("[FeatureFlags] ── Sprint 14~16 신규 기능 (Phase-Gate) ──");
        log.info("[FeatureFlags]   F-20 provisioning        = {}  (IDEM_HUB_PROVISIONING_ENABLED)  Phase2↑", fmt(provisioning));
        log.info("[FeatureFlags]   F-21 provisioningRelay   = {}  (IDEM_HUB_PROVISIONING_RELAY_ENABLED) Phase2B↑", fmt(provisioningRelay));
        log.info("[FeatureFlags]   F-22 provisioningDryRun  = {}  (IDEM_HUB_PROVISIONING_DRY_RUN)  Phase2A=true", fmt(provisioningDryRun));
        log.info("[FeatureFlags]   F-23 gatewayInbound      = {}  (IDEM_HUB_GATEWAY_INBOUND_ENABLED) Phase3↑", fmt(gatewayInbound));
        log.info("[FeatureFlags]   F-24 gatewayOutbound     = {}  (IDEM_HUB_GATEWAY_OUTBOUND_ENABLED) Phase3B↑", fmt(gatewayOutbound));
        log.info("[FeatureFlags]   F-25 gatewayIdempotency  = {}  (IDEM_HUB_GATEWAY_IDEMPOTENCY_ENABLED) 항상ON", fmt(gatewayIdempotency));
        log.info("[FeatureFlags]   F-26 hmacSigRequired     = {}  (IDEM_HUB_HMAC_SIG_REQUIRED)  Phase4↑(S17)", fmt(hmacSigRequired));
        log.info("[FeatureFlags]   F-27 agencyKeyAuditLog   = {}  (IDEM_HUB_AGENCY_KEY_AUDIT_LOG) 항상ON", fmt(agencyKeyAuditLog));
        log.info("[FeatureFlags] ==========================================");

        // ── 운영 위험 경고 ──
        if (!auditDb) {
            log.warn("[FeatureFlags] ⚠️  F-04 auditDb=OFF — 감사 로그 DB 저장 비활성. 운영 환경에서는 IDEM_HUB_AUDIT_DB_ENABLED=true 필수!");
        }
        if (retentionJob && !retentionDryRun) {
            log.warn("[FeatureFlags] ⚠️  F-11 retentionJob=ON + dryRun=OFF — 개인정보 실제 파기 활성. 법무팀 승인 확인 필요!");
        }
        if (!redissonLock) {
            log.warn("[FeatureFlags] ⚠️  F-08 redissonLock=OFF — 분산 락 비활성. K8s 다중 Pod 환경에서는 IDEM_HUB_REDISSON_ENABLED=true 필수!");
        }
        ...
        if (provisioning && !provisioningDryRun && !provisioningRelay) {
            log.warn("[FeatureFlags] ⚠️  F-20 ON + F-21 OFF: 프로비저닝 실패 시 재시도 릴레이 없음. IDEM_HUB_PROVISIONING_RELAY_ENABLED=true 권장.");
        }
        if (gatewayInbound && !gatewayIdempotency) {
            log.warn("[FeatureFlags] ⚠️  F-23 ON + F-25 OFF: Gateway 인바운드 멱등성 방어 비활성. 중복 이벤트 위험. IDEM_HUB_GATEWAY_IDEMPOTENCY_ENABLED=true 필수!");
        }
        if (hmacSigRequired) {
     ...
     *     "F-20_provisioning": {
     *       "enabled": false,
     *       "env": "IDEM_HUB_PROVISIONING_ENABLED",
     *       "phase": "Phase2",
    ...
    public java.util.Map<String, Object> toStatusMap() {
        return java.util.Map.ofEntries(
            ...
            java.util.Map.entry("F-01_authRateLimit",
                featureEntry(authRateLimit, "IDEM_HUB_AUTH_RL_ENABLED", "stable", "IP 기반 Auth 엔드포인트 Rate Limiting")),
            java.util.Map.entry("F-02_agencyRateLimit",
                featureEntry(agencyRateLimit, "IDEM_HUB_RATE_LIMIT_ENABLED", "stable", "기관별 API Rate Limiting (Bucket4j)")),
            ...
            java.util.Map.entry("F-03_auditKafka",
                featureEntry(auditKafka, "IDEM_HUB_AUDIT_KAFKA_ENABLED", "stable", "감사 로그 Kafka 비동기 발행")),
            java.util.Map.entry("F-04_auditDb",
                featureEntry(auditDb, "IDEM_HUB_AUDIT_DB_ENABLED", "stable", "감사 로그 DB 저장 (OFF금지)")),
            java.util.Map.entry("F-05_authTracing",
                featureEntry(authTracing, "IDEM_HUB_AUTH_TRACING_ENABLED", "stable", "OTel 분산 추적 AOP")),
            java.util.Map.entry("F-08_redissonLock",
                featureEntry(redissonLock, "IDEM_HUB_REDISSON_ENABLED", "stable", "Redisson 분산 락")),
            java.util.Map.entry("F-10_securityHeaders",
                featureEntry(securityHeaders, "IDEM_HUB_SECURITY_HEADERS_ENABLED", "stable", "보안 응답 헤더 필터")),
            java.util.Map.entry("F-11_retentionJob",
                featureEntry(retentionJob, "IDEM_HUB_RETENTION_ENABLED", "stable", "개인정보 파기 스케줄러")),
            java.util.Map.entry("F-11b_retentionDryRun",
                featureEntry(retentionDryRun, "IDEM_HUB_RETENTION_DRY_RUN", "stable", "파기 dry-run 모드")),
            java.util.Map.entry("F-12_cryptoRotation",
                featureEntry(cryptoRotation, "IDEM_HUB_CRYPTO_ROTATION_ENABLED", "stable", "Handoff AES 키 로테이션")),
            java.util.Map.entry("F-13_outboxRelay",
                featureEntry(outboxRelay, "IDEM_HUB_OUTBOX_RELAY_ENABLED", "stable", "IdO Outbox Kafka 릴레이")),
            java.util.Map.entry("F-14_webhookRelay",
                featureEntry(webhookRelay, "IDEM_HUB_WEBHOOK_RELAY_ENABLED", "stable", "Webhook Outbox HTTP 릴레이")),
            java.util.Map.entry("F-18_spReceiverAudit",
                featureEntry(spReceiverAudit, "IDEM_HUB_REGISTRY_RECEIVER_AUDIT", "stable", "Q-IM SP 수신 감사 로그")),

            // ── Sprint 14~16 신규 기능 (Phase-Gate) ──
            java.util.Map.entry("F-20_provisioning",
                featureEntry(provisioning, "IDEM_HUB_PROVISIONING_ENABLED", "Phase2", "전 기관 Virtual Thread 병렬 프로비저닝")),
            java.util.Map.entry("F-21_provisioningRelay",
                featureEntry(provisioningRelay, "IDEM_HUB_PROVISIONING_RELAY_ENABLED", "Phase2B", "Provisioning Outbox 지수백오프 릴레이")),
            java.util.Map.entry("F-22_provisioningDryRun",
                featureEntry(provisioningDryRun, "IDEM_HUB_PROVISIONING_DRY_RUN", "Phase2A", "프로비저닝 dry-run (로그만, HTTP 미발행)")),
            java.util.Map.entry("F-23_gatewayInbound",
                featureEntry(gatewayInbound, "IDEM_HUB_GATEWAY_INBOUND_ENABLED", "Phase3A", "기관→OnePass 인바운드 이벤트 API")),
            java.util.Map.entry("F-24_gatewayOutbound",
                featureEntry(gatewayOutbound, "IDEM_HUB_GATEWAY_OUTBOUND_ENABLED", "Phase3B", "OnePass→기관 아웃바운드 API")),
            java.util.Map.entry("F-25_gatewayIdempotency",
                featureEntry(gatewayIdempotency, "IDEM_HUB_GATEWAY_IDEMPOTENCY_ENABLED", "always-on", "Gateway Redis 멱등성 중복 방어")),
            java.util.Map.entry("F-26_hmacSigRequired",
                featureEntry(hmacSigRequired, "IDEM_HUB_HMAC_SIG_REQUIRED", "Phase4(S17)", "X-Internal-Sig HMAC-SHA256 필수 검증")),
            java.util.Map.entry("F-27_agencyKeyAuditLog",
                featureEntry(agencyKeyAuditLog, "IDEM_HUB_AGENCY_KEY_AUDIT_LOG", "always-on", "Agency API Key 인증 감사 로그"))
        );
    }
