...
CREATE SCHEMA IF NOT EXISTS idem_authz;

-- ── 1. 역할 카탈로그 ────────────────────────────────────────────────────────
-- 기관별 네임스페이스. agency_code='PLATFORM' = 전역 역할(소수 고정).
CREATE TABLE IF NOT EXISTS idem_authz.authz_role (
...
COMMENT ON TABLE  idem_authz.authz_role            IS '기관별 역할 카탈로그(의미 불투명). PLATFORM=전역 역할.';
COMMENT ON COLUMN idem_authz.authz_role.role_code  IS '기관이 자유 정의하는 역할 코드. 플랫폼은 의미를 해석하지 않음.';

-- ── 2. 사용자 역할 부여 (assignment) — ★ 핵심 SoR ──────────────────────────
CREATE TABLE IF NOT EXISTS idem_authz.authz_user_role (
    ...
    CONSTRAINT fk_authz_ur_role FOREIGN KEY (agency_code, role_code)
        REFERENCES idem_authz.authz_role(agency_code, role_code) ON DELETE RESTRICT
);

-- 사용자×기관 유효권한 조회(토큰 클레임 발급 핵심 경로)
CREATE INDEX IF NOT EXISTS idx_authz_ur_user_agency
    ON idem_authz.authz_user_role(qim_user_id, agency_code) WHERE status = 'ACTIVE';
-- 기관 스코프 회원/역할 조회
CREATE INDEX IF NOT EXISTS idx_authz_ur_agency_role
    ON idem_authz.authz_user_role(agency_code, role_code) WHERE status = 'ACTIVE';
-- 만료 배치 스캔
CREATE INDEX IF NOT EXISTS idx_authz_ur_expires
    ON idem_authz.authz_user_role(expires_at) WHERE status = 'ACTIVE' AND expires_at IS NOT NULL;

COMMENT ON TABLE  idem_authz.authz_user_role        IS '사용자 역할 부여(중앙 SoR). 토큰 roles[] 클레임의 원천.';
COMMENT ON COLUMN idem_authz.authz_user_role.status IS 'ACTIVE: 유효, REVOKED: 회수, EXPIRED: 만료(스케줄러 전이)';

-- ── 3. append-only 부여 감사 (100% 기록) ───────────────────────────────────
CREATE TABLE IF NOT EXISTS idem_authz.authz_grant_audit (
    id             UUID         NOT NULL DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_authz_audit_user   ON idem_authz.authz_grant_audit(qim_user_id, at DESC);
CREATE INDEX IF NOT EXISTS idx_authz_audit_agency ON idem_authz.authz_grant_audit(agency_code, at DESC);

COMMENT ON TABLE idem_authz.authz_grant_audit IS '인가 부여/회수 append-only 감사 이력(법적·보안 추적).';
...
ALTER TABLE idem_authz.authz_role           ENABLE ROW LEVEL SECURITY;
ALTER TABLE idem_authz.authz_user_role      ENABLE ROW LEVEL SECURITY;
ALTER TABLE idem_authz.authz_grant_audit    ENABLE ROW LEVEL SECURITY;

CREATE POLICY rls_authz_role_tenant ON idem_authz.authz_role
...
CREATE POLICY rls_authz_user_role_tenant ON idem_authz.authz_user_role
    USING (current_setting('app.current_agency', TRUE) IS NULL
           OR agency_code = current_setting('app.current_agency', TRUE));

CREATE POLICY rls_authz_grant_audit_tenant ON idem_authz.authz_grant_audit
    USING (current_setting('app.current_agency', TRUE) IS NULL
           OR agency_code = current_setting('app.current_agency', TRUE));
