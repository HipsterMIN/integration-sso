...
CREATE TABLE IF NOT EXISTS authz.authz_assignment (
...
CREATE INDEX IF NOT EXISTS idx_authz_assignment_agency
    ON authz.authz_assignment(agency_code) WHERE status = 'ACTIVE';
CREATE INDEX IF NOT EXISTS idx_authz_assignment_expires
    ON authz.authz_assignment(expires_at) WHERE status = 'ACTIVE' AND expires_at IS NOT NULL;
COMMENT ON TABLE  authz.authz_assignment        IS '사용자 ↔ Service 할당(중앙 SoR). hub ASSIGNMENT 정책 규칙·Handoff/CAST 발급의 원천.';
COMMENT ON COLUMN authz.authz_assignment.source IS 'ROLE_GRANT: 역할 부여로 자동 생성, SELF_SIGNUP: 기관 셀프 가입 완료 보고, AGENCY_PUSH: 기관 API';

ALTER TABLE authz.authz_assignment ENABLE ROW LEVEL SECURITY;
CREATE POLICY rls_authz_assignment_tenant ON authz.authz_assignment
    USING (current_setting('app.current_agency', TRUE) IS NULL
           OR agency_code = current_setting('app.current_agency', TRUE));

-- 감사 이벤트에 할당/해제 추가
ALTER TABLE authz.authz_grant_audit DROP CONSTRAINT IF EXISTS chk_authz_audit_event;
ALTER TABLE authz.authz_grant_audit ADD CONSTRAINT chk_authz_audit_event
...
INSERT INTO authz.authz_assignment (qim_user_id, agency_code, status, source, granted_at, granted_by, reason)
SELECT DISTINCT ur.qim_user_id, ur.agency_code, 'ACTIVE', 'ROLE_GRANT', NOW(), 'SYSTEM', 'S8-b 백필: 활성 역할 보유'
FROM authz.authz_user_role ur
