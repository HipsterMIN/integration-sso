...
--     idem_authz.assignment.events 토픽으로 발행(ShedLock 다중 Pod 보호)
--   · payload는 text — 아웃박스는 상태/생성순으로만 조회하는 전송 큐이므로
--     jsonb 질의·색인이 불필요(String 바인딩 단순화)
-- ============================================================================

CREATE TABLE IF NOT EXISTS idem_authz.authz_outbox (
    ...
    topic          VARCHAR(200)  NOT NULL DEFAULT 'idem_authz.assignment.events',
...
CREATE INDEX IF NOT EXISTS idx_authz_outbox_status
    ON idem_authz.authz_outbox (status, created_at);

COMMENT ON TABLE  idem_authz.authz_outbox            IS '인가 이벤트 트랜잭셔널 아웃박스 — relay-batch가 idem_authz.assignment.events로 발행.';
COMMENT ON COLUMN idem_authz.authz_outbox.partition_key IS 'qimUserId — 사용자 단위 이벤트 순서 보장(Kafka 파티션 키).';
COMMENT ON COLUMN idem_authz.authz_outbox.aggregate_id  IS 'agency_code:role_code — 부여 대상 식별.';
