...
/**
 * 기관 샘플의 표준 OIDC RP 설정 (S6 PR-2) — {@code agency-stub.oidc.*}.
 *
 * <p>기관이 Idem 에서 받는 것은 {@code issuer·clientId·clientSecret} 셋뿐이다. 나머지는 {@code {issuer}/.well-known/openid-configuration} 이 알려 준다.
 * {@code agency-stub.protocol=OIDC_RP} 일 때만 {@code /agency/oidc/**} 가 뜻을 가진다(Handoff 진입은 그대로 남아 있다).
 */
@Getter
@Setter
@Component
@ConfigurationProperties(prefix = "agency-stub.oidc")
