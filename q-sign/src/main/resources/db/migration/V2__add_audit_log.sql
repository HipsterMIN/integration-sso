...
-- ──────────────────────────────────────────────────────────────
CREATE TABLE qsign.auth_audit_log (
...
CREATE INDEX idx_audit_log_correlation   ON qsign.auth_audit_log (correlation_id);
CREATE INDEX idx_audit_log_identifier    ON qsign.auth_audit_log (identifier_hash, occurred_at DESC);
CREATE INDEX idx_audit_log_event_type    ON qsign.auth_audit_log (event_type, occurred_at DESC);
CREATE INDEX idx_audit_log_occurred_at   ON qsign.auth_audit_log (occurred_at DESC);

COMMENT ON TABLE  qsign.auth_audit_log             IS '§15 인증 감사 로그 – 불변 이력 (DELETE/UPDATE 금지)';
COMMENT ON COLUMN qsign.auth_audit_log.audit_key   IS 'SHA-256(correlationId ‖ authResultId ‖ ts) 위변조 검증용';
COMMENT ON COLUMN qsign.auth_audit_log.client_ip   IS 'PII 주의 – 마스킹 정책 적용 대상';

-- ──────────────────────────────────────────────────────────────
-- 2. 잠금 이벤트 이력 (LockEvent)
--    auth_lock 의 상태 전이를 감사 목적으로 별도 기록.
-- ──────────────────────────────────────────────────────────────
CREATE TABLE qsign.lock_event_log (
...
CREATE INDEX idx_lock_event_key  ON qsign.lock_event_log (lock_key, occurred_at DESC);
CREATE INDEX idx_lock_event_type ON qsign.lock_event_log (event_type, occurred_at DESC);

COMMENT ON TABLE qsign.lock_event_log IS '잠금 상태 전이 감사 이력';
