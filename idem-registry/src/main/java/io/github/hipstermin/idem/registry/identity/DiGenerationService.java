 ...
 * </ul>
 *
 * <p>환경변수: {@code IDEM_REGISTRY_DI_SECRET} (서비스 공통 비밀키, 운영 시 반드시 교체)
 */
@Slf4j
@Component
...
public class DiGenerationService {
     ...
     * 환경변수: {@code IDEM_REGISTRY_DI_SECRET} — 운영 환경 필수.
     *
     * <p>[Sprint γ-1 / F3.4] 기본값 "default-di-secret-change-in-production" 제거.
     * 부팅 시 {@link #validateDiSecret()} 가 placeholder 또는 비어있을 경우 즉시 실패.
     */
    @Value("${idem.registry.crypto.di.secret:}")
    private String diSecret;

    /**
     * 부팅 검증 우회 escape hatch — 테스트/로컬 한정.
     * 운영 환경에서는 절대 true 설정 금지.
     * <p>활성화 방법(테스트 한정): {@code idem.registry.crypto.di.allow-empty-secret=true}
     */
    @Value("${idem.registry.crypto.di.allow-empty-secret:false}")
    ...
    void validateDiSecret() {
        if (diSecret == null || diSecret.isBlank()) {
            if (allowEmptyDiSecret) {
                log.warn("[DiGenerationService] idem.registry.crypto.di.secret 미설정 — "
                        + "allow-empty-secret=true 로 우회 (테스트/로컬 한정)");
                return;
            }
            throw new IllegalStateException(
                    "idem.registry.crypto.di.secret 환경변수 IDEM_REGISTRY_DI_SECRET 가 설정되지 않았습니다. "
                            + "운영에서는 반드시 32자 이상의 무작위 비밀키를 주입하십시오 "
                            + "(예: openssl rand -hex 32). "
                            + "테스트/로컬에서만 idem.registry.crypto.di.allow-empty-secret=true 로 우회 가능.");
        }
        String normalized = diSecret.trim().toLowerCase();
        if (FORBIDDEN_PLACEHOLDERS.contains(normalized)) {
            throw new IllegalStateException(
                    "idem.registry.crypto.di.secret 가 안전하지 않은 placeholder('" + diSecret + "') 입니다. "
                            + "운영용 비밀키를 주입하십시오. "
                            + "(이 값으로 운영하면 모든 기관의 DI 가 공개 사전 공격에 노출됩니다.)");
        }
        if (diSecret.length() < 16) {
            throw new IllegalStateException(
                    "idem.registry.crypto.di.secret 가 너무 짧습니다 (length=" + diSecret.length() + "). "
