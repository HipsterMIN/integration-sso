...
function useSubPageAnimation(): void {
	useEffect(() => {
		...
		// 마운트 시점엔 모든 form-container 자식에 .show 를 즉시 부여 — viewport 밖 카드도 표시되도록.
		// 스크롤 진입 애니메이션은 applyDelay + fade_up 키프레임이 stagger 로 처리.
		document
			.querySelectorAll<HTMLElement>('.sub .form-container > *')
			.forEach((section) => section.classList.add('show'));

		// 동적으로 추가되는 요소(예: 비동기 로딩 후 렌더) 대비 — 스크롤 핸들러는 그대로 유지
...
function ConversionLayout({
	...
	onNext,
	onSkip,
}: ConversionLayoutProps): JSX.Element {
	const steps = getSteps(memberType);
	...
	return (
		<main id="main-content" className={cx('container', 'sub', 'conversion', `step${currentStep}`, { business: isBusiness })}>
