...
CustomTimePickerProps): JSX.Element {
	...
	const [customTime, setCustomTime] = useState<RangePickerProps['value']>([
		null,
		null,
	]);
	...
	useEffect(() => {
		let value = '';
		value = getSelectedTimeRangeLabel(selectedTime, selectedValue);
		setSelectedTimePlaceholderValue(value);

		if (selectedTime !== 'custom') setCustomTime([null, null]);
	}, [selectedTime, selectedValue, selectedRadioTime]);
	...
	return (
		<div className="custom-time-picker">
				...
				content={
					newPopover ? (
						<CustomTimePickerPopoverContent
							...
							selectedTimeLabel={selectedTimePlaceholderValue}
			...
			>
				<Input
					// 240930 realtime active(true) 일 경우 value = '-' / color 변경 (화면설계서 p12참고)
					className={cx(
						isRealTime ? 'timeSelection-input' : 'timeSelection-input active',
					)}
					type="text"
					...
					value={isRealTime ? '-' : selectedTimePlaceholderValue}
					onFocus={handleFocus}
					onBlur={handleBlur}
