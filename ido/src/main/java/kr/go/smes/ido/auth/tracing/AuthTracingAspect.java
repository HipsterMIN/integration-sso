 ...
 *
 * <p>NICE/OACX/CI 처리 흐름에서 커스텀 스팬을 생성하여 분산 추적 가시성을 높인다.
 ...
 *   <li>{@code NiceAuthService.ensureAccessToken()} — NICE 토큰 갱신 스팬</li>
...
public class AuthTracingAspect {
    ...
    /**
     * 메서드 실행을 스팬으로 감싸는 공통 로직
     *
     * <p>현재 활성 스팬의 자식 스팬으로 생성하여 분산 추적 계층 구조 유지.
     * {@code Tracer}가 null이거나 스팬 생성 실패 시 원본 메서드를 그대로 실행 (추적 실패가 비즈니스 실패로 전파되지 않음).
     *
    ...
    private Object traceMethod(ProceedingJoinPoint pjp, String spanName,
        ...
        try (Tracer.SpanInScope ws = tracer.withSpan(span)) {
            ...
            // 메서드 시그니처가 비즈니스 오류를 반환값으로 처리하므로
            // 예외가 발생했을 때만 error로 기록
            span.error(t);
            throw t;
        } finally {
            span.end();
