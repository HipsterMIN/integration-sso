...
-- 설계서 §16.3 / §11.5.5 참조
--
-- IdempotentEventStore (ido.processed_event):
...
-- ──────────────────────────────────────────────────────────────
-- 1. 처리 완료 이벤트 (ido.processed_event)
--    §16.3 멱등 컨슈머 중복 방지.
...
COMMENT ON COLUMN ido.processed_event.event_id
    IS 'DomainEvent.eventId (UUID)';
COMMENT ON COLUMN ido.processed_event.consumer_group
    IS 'Kafka consumer group ID (ido-qim-consumer 등)';
COMMENT ON COLUMN ido.processed_event.result_code
    IS 'OK = 정상 처리 / SKIPPED = 버전 역전·중복 / ERROR = 처리 실패';

-- ──────────────────────────────────────────────────────────────
-- 2. 이벤트 버전 추적 (ido.last_event_version)
...
COMMENT ON COLUMN ido.last_event_version.aggregate_id
