...
COMMENT ON COLUMN ido.agency_meta.callback_whitelist IS '§12.3 returnUrl 화이트리스트 JSON 배열';
COMMENT ON COLUMN ido.agency_meta.allowed_attributes IS '기관이 조회 허용된 사용자 속성 목록';
COMMENT ON COLUMN ido.agency_meta.integration_type   IS '§13 연동 패턴: DIRECT / APACHE_GATE / BRIDGE / INTERNAL_SSO';
...
COMMENT ON COLUMN ido.handoff_audit.ticket_id    IS 'Redis key 와 동일한 UUIDv4 – 단방향 추적 가능';
COMMENT ON COLUMN ido.handoff_audit.state        IS 'ISSUED → CONSUMED(정상) | EXPIRED(TTL만료) | REVOKED(강제취소)';
...
COMMENT ON TABLE ido.last_event_version IS '§11.5.5 Q-IM 이벤트 버전 추적 – Ordered Consumer 패턴';
