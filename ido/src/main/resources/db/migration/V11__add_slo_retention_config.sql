...
COMMENT ON TABLE  ido.personal_data_retention_log                  IS '개인정보 보존 기간 만료 파기 이력 (§15.2 개인정보 보호 / Sprint 2 P1-04)';
COMMENT ON COLUMN ido.personal_data_retention_log.log_id           IS '파기 이력 ID (UUIDv4)';
COMMENT ON COLUMN ido.personal_data_retention_log.inst_mbr_id      IS '파기된 기관 회원 ID';
COMMENT ON COLUMN ido.personal_data_retention_log.qim_user_id      IS 'Q-IM 사용자 ID (파기 시점 기록)';
COMMENT ON COLUMN ido.personal_data_retention_log.purge_status     IS 'SUCCESS / FAILED';
COMMENT ON COLUMN ido.personal_data_retention_log.purge_detail     IS '파기 상세 내용 또는 실패 원인';
COMMENT ON COLUMN ido.personal_data_retention_log.retention_cutoff IS '적용된 보존 기간 만료 기준 시각';
COMMENT ON COLUMN ido.personal_data_retention_log.correlation_id   IS '스케줄 실행 추적 ID';
...
COMMENT ON TABLE  ido.slo_audit_log                  IS 'SLO(Single Logout) 실행 감사 로그 (§13.3 / Sprint 2 P1-01~03)';
COMMENT ON COLUMN ido.slo_audit_log.slo_id           IS 'SLO 이벤트 ID (UUIDv4)';
COMMENT ON COLUMN ido.slo_audit_log.qim_user_id      IS '로그아웃한 사용자 Q-IM ID';
COMMENT ON COLUMN ido.slo_audit_log.fe_session_id    IS '만료된 FE 세션 ID';
COMMENT ON COLUMN ido.slo_audit_log.keycloak_status  IS 'SUCCESS / FAILED / SKIPPED';
COMMENT ON COLUMN ido.slo_audit_log.webhook_status   IS 'ENQUEUED / SKIPPED / FAILED';
COMMENT ON COLUMN ido.slo_audit_log.correlation_id   IS '추적 ID';
