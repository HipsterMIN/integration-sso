...
COMMENT ON COLUMN ido.auth_result.auth_method
...
COMMENT ON COLUMN ido.auth_result.issued_at
...
COMMENT ON COLUMN ido.auth_result.expires_at
...
COMMENT ON COLUMN ido.auth_result.raw_id_token
...
CREATE INDEX IF NOT EXISTS idx_auth_result_auth_method
    ON ido.auth_result (auth_method)
    WHERE auth_method IS NOT NULL;

-- ─────────────────────────────────────────────────────────────────────────────
-- 2. ido.provider_circuit_config — provider_code 단위 회로차단기 동적 설정
...
CREATE TABLE IF NOT EXISTS ido.provider_circuit_config (
    ...
    CONSTRAINT fk_pcc_provider_code
        FOREIGN KEY (provider_code) REFERENCES ido.provider_config (provider_code)
        ON UPDATE CASCADE ON DELETE CASCADE
);

COMMENT ON TABLE ido.provider_circuit_config
